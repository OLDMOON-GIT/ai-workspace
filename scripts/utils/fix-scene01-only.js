/**
 * scene_01만 1080p 25fps로 변환 후 concat demuxer로 빠르게 병합
 */
...
// Step 1: scene_01만 1080p 25fps로 변환
console.log('Step 1: scene_01 변환 (720p 24fps -> 1080p 25fps)');
console.log('-'.repeat(70));
...
execSync(
  ...
  `-c:v libx264 -preset veryfast -crf 23 -c:a aac -b:a 192k -ar 48000 "${scene01Fixed}"`,
  { stdio: 'inherit' }
);

console.log('\n✅ scene_01 변환 완료\n');

// Step 2: concat list 생성 (변환된 scene_01 + 나머지)
...
// Step 3: concat demuxer로 빠른 병합
console.log('Step 3: concat demuxer로 병합 (빠름!)');
console.log('-'.repeat(70));

const outputPath = path.join(PROJECT_DIR, 'merged_fixed.mp4');
