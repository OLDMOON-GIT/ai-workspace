...
async function updateScenario() {
  ...
  const testCode = `// [자동] tasks 페이지 로드 확인
// 자동 감지: app\\admin\\tasks\\page.tsx - 페이지가 정상적으로 로드되는지 확인
...
// 페이지 제목 또는 주요 요소 확인
await page.waitForSelector('h1:has-text("작업 관리")', { timeout: 15000 });
`;
  ...
  console.log('Scenario 170 updated successfully');
  await conn.end();
}
