...
### 사용자에게 작업을 시키지 않는다
- 문제 해결을 위해 사용자에게 확인, 테스트, 명령어 실행 등을 요청하지 않습니다
- AI가 직접 코드를 확인하고, 로그를 분석하고, 문제를 해결합니다
- 사용자는 결과만 확인하면 됩니다

### Claude Code 설치 및 사용
- **절대로 npm 버전으로 설치하지 않는다**
  - npm 버전 (`npm install -g @anthropic-ai/claude-code`)은 사용하지 않습니다
  - 공식 설치 프로그램을 사용하여 설치합니다
  - 이유: npm 버전은 성능 문제와 설정 이슈가 있습니다
...
사용자 경험 일관성을 위해 "내 콘텐츠" 페이지의 모든 버튼은 **보기 | 제작 | 편집** 순서로 배치합니다.

### 13.2 대본 버튼 순서

**위치:** `trend-video-frontend/src/app/my-content/page.tsx`

**대본 (completed 상태) 버튼 배치:**
...
{script.status === 'completed' && (
  <>
    ...
    {/* === 제작 === */}
    <button>🎨 이미지크롤링</button>
    <button>🎬 영상</button>
    {script.type === 'product' && (
      <button>🛍️ 상품정보</button>
    )}
...
1. **이모티콘 일관성**
   - `✨ 포멧팅`: 반짝임 효과로 정렬 기능 강조
   - `✏️ 편집`: 연필 아이콘으로 수정 기능 직관적 표현
   - `📥 다운로드`: "저장"에서 "다운로드"로 명칭 변경

2. **관리자 전용 버튼**
   - `✏️ 편집`, `📁 폴더`는 `user?.isAdmin`일 때만 표시
   - 편집 모드 진입 시 해당 버튼 숨김 (`editingScriptId !== script.id`)

3. **조건부 버튼**
   - `📋 로그`: `logs && logs.length > 0`
   - `🛍️ 상품정보`: `type === 'product'`
   - `⚡ 쇼츠`: `type === 'longform'`
...
- `trend-video-frontend/src/app/my-content/page.tsx`
  - "모든" 탭 대본 버튼: lines ~2720-2895
  - "대본" 탭 버튼: lines ~3471-3716
  - "영상" 탭 버튼: lines ~4040-4180
