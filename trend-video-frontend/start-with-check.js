...
async function startUnifiedWorker() {
  console.log('🚀 통합 워커 시작 중 (Script + Image + Video + YouTube)...\n');

  // 같은 프로세스 내에서 워커 실행
  const UnifiedWorker = require('./src/workers/unified-worker.js');
  const worker = new UnifiedWorker();

  // 워커를 프로세스 내에서 실행 (블로킹하지 않음)
  worker.start().catch(err => {
    console.error('❌ 통합 워커 실행 중 오류:', err);
  });

  console.log('✅ 통합 워커가 프로세스 내에서 실행됩니다.');
...
async function startServer() {
  ...
  // 통합 워커 시작 (같은 프로세스 내에서)
  await startUnifiedWorker();
  ...
  const npmArgs = ['run', 'dev:server-only'];
  if (extraArgs.length > 0) {
    npmArgs.push('--');
    npmArgs.push(...extraArgs);
  }

  const devProcess = spawn('npm', npmArgs, {
