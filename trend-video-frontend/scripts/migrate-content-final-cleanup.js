/**
 * content 테이블 최종 정리 - 중복/불필요한 컬럼 제거
 *
 * 제거 컬럼:
 * - task_id: content_id = task_id로 중복
 * - type: script_content/video_path 존재로 판단
 * - format: prompt_format으로 통일
 */
...
function finalCleanup() {
  ...
  try {
    ...
    console.log('🚀 content 테이블 최종 정리 시작');
    ...
    // 제거할 컬럼 확인
    const columnsToRemove = ['task_id', 'type', 'format'];
    const existingColumns = columns.map(c => c.name);
    const actuallyRemove = columnsToRemove.filter(col => existingColumns.includes(col));

    console.log('\n❌ 제거할 컬럼:', actuallyRemove.join(', '));

    if (actuallyRemove.length === 0) {
      console.log('✅ 제거할 컬럼이 없습니다. 이미 정리되었습니다.');
    ...
    db.exec(`
      ...
      SELECT
        content_id,
        user_id,
        COALESCE(prompt_format, format) as prompt_format,  -- format 폴백
        title,
        original_title,
        script_content,
        video_path,
