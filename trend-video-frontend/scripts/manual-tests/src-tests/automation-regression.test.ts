...
describe('자동화 파이프라인 회귀 테스트', () => {
  ...
  function setupTestData() {
    ...
    try {
      db.prepare(`
        INSERT INTO youtube_channel_setting (id, user_id, channel_id, channel_name, created_at)
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
  ...
  function cleanupTestData() {
    try {
      ...
      db.prepare('DELETE FROM task_schedule WHERE id LIKE ?').run('schedule_test_%');
      ...
      db.prepare('DELETE FROM youtube_channel_setting WHERE id = ?').run(testChannelSettingsId);
    } catch (e) {
  ...
  describe('1. 데이터베이스 스키마', () => {
    ...
    test('task_schedule 테이블이 youtube_upload_id 컬럼을 가져야 함', () => {
      const tableInfo = db.prepare('PRAGMA table_info(task_schedule)').all() as any[];
      const columnNames = tableInfo.map(col => col.name);
  ...
  describe('2. 채널 설정 조회', () => {
    test('youtube_channel_setting에서 channel_id를 정상적으로 조회할 수 있어야 함', () => {
    ...
    test('video_titles와 youtube_channel_setting를 JOIN하여 채널 정보를 가져올 수 있어야 함', () => {
      // 테스트 타이틀 생성
      ...
      const result = db.prepare(`
        ...
        FROM video_titles t
        LEFT JOIN youtube_channel_setting yc ON t.channel = yc.id
        WHERE t.id = ?
      `).get(titleId) as any;
  ...
  describe('3. 스케줄 조회', () => {
    test('getPendingSchedules 쿼리가 모든 필요한 필드를 반환해야 함', () => {
      ...
      db.prepare(`
        INSERT INTO task_schedule (id, title_id, scheduled_time, status, created_at)
      ...
      // 정리
      db.prepare('DELETE FROM task_schedule WHERE id = ?').run(scheduleId);
      db.prepare('DELETE FROM video_titles WHERE id = ?').run(titleId);
    });
  });
  ...
  describe('4. YouTube 업로드 데이터 저장', () => {
    test('youtube_uploads 테이블에 데이터를 저장할 수 있어야 함', () => {
      ...
      const taskId = 'job_test_' + Date.now();
    ...
    test('task_schedule에 youtube_upload_id를 업데이트할 수 있어야 함', () => {
      const titleId = 'title_test_' + Date.now();
      const scheduleId = 'schedule_test_' + Date.now();
      const uploadId = 'upload_test_' + Date.now();
      ...
      const schedule = db.prepare('SELECT * FROM task_schedule WHERE id = ?').get(scheduleId) as any;
  ...
  describe('6. 비디오 파일 경로', () => {
    test('jobs 테이블의 video_path 컬럼을 사용해야 함', () => {
      ...
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
      `).run(taskId, 'Test Job', 'completed', videoPath);

      const job = db.prepare('SELECT * FROM jobs WHERE id = ?').get(taskId) as any;
      ...
      // 정리
      db.prepare('DELETE FROM jobs WHERE id = ?').run(taskId);
    });
  });
  ...
  describe('9. 전체 파이프라인 시뮬레이션', () => {
    test('완전한 파이프라인 플로우를 시뮬레이션할 수 있어야 함', () => {
      ...
      // 2. Schedule 생성
      db.prepare(`
        INSERT INTO task_schedule (id, title_id, scheduled_time, youtube_publish_time, status, created_at)
