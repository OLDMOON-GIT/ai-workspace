...
function initYouTubeDB() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS youtube_uploads (
      ...
      FOREIGN KEY (channel_id) REFERENCES youtube_channel_setting(setting_id)
    );
  ...
  describe('Suite 1: YouTube 채널 관리', () => {
    test('✅ YouTube 채널 등록', () => {
      ...
      const channel = db.prepare(`
        SELECT * FROM youtube_channel_setting WHERE setting_id = ?
    ...
    test('✅ 채널별 설정 관리', () => {
      ...
      const allChannels = db.prepare(`
        SELECT * FROM youtube_channel_setting WHERE user_id = 'user-001'
      `).all() as any[];
