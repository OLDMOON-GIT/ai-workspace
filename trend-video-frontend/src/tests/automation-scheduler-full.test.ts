...
const hasTaskSchedule = !!db
  .prepare(`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'task_schedule'`)
  .get();
const scheduleDescribe = hasTaskSchedule ? describe : describe.skip;
...
scheduleDescribe('⚙️ 자동화 스케줄러 전체 시나리오 통합 테스트', () => {
  ...
  describe('Suite 1: 스케줄 상태 전환', () => {
    test('✅ pending → processing → completed 상태 전환', () => {
      ...
      let schedule = db.prepare('SELECT * FROM task_schedule WHERE id = ?').get(scheduleId) as any;
      ...
      schedule = db.prepare('SELECT * FROM task_schedule WHERE id = ?').get(scheduleId) as any;
      ...
      // 3. 진행률 업데이트
      db.prepare(`
        UPDATE task_schedule SET progress = 50 WHERE id = ?
      `).run(scheduleId);
  ...
  describe('Suite 4: YouTube 채널 및 업로드 준비', () => {
    ...
    test('✅ 다양한 프라이버시 설정', () => {
      ...
      const results = db.prepare(`
        SELECT youtube_privacy, COUNT(*) as count FROM task_schedule
  ...
  describe('Suite 5: 복합 시나리오', () => {
    test('✅ 완전한 스케줄 처리 흐름 (상품 → 스크립트 → 비디오 → 업로드 준비)', () => {
      ...
      // 3. 로그 기록
      db.prepare(`
        UPDATE task_schedule SET status = 'processing', started_at = CURRENT_TIMESTAMP WHERE id = ?
      `).run(scheduleId);
      ...
      // 5. 업로드 준비 상태로 변경
      db.prepare(`
        UPDATE task_schedule SET status = 'ready_for_upload', progress = 100 WHERE id = ?
      `).run(scheduleId);
    ...
    test('✅ 다중 스케줄 동시 처리', () => {
      ...
      const allSchedules = db.prepare(`
        SELECT COUNT(*) as count FROM task_schedule WHERE id IN (${scheduleIds.map(() => '?').join(',')})
  ...
  describe('Suite 6: 예외 처리 및 복구', () => {
    ...
    test('✅ 최대 재시도 횟수 초과 처리', () => {
      ...
      const current = db.prepare('SELECT retry_count FROM task_schedule WHERE id = ?').get(scheduleId) as any;
