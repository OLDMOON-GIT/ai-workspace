...
describe('🛍️ 자동화 상품카테고리 통합 테스트', () => {
  ...
  const scheduleCount = () =>
    hasTaskSchedule ? (db.prepare('SELECT COUNT(*) as cnt FROM task_schedule').get()).cnt : 0;
  ...
  beforeAll(() => {
    console.log('\n🔧 테스트 환경 초기화 중...');
    db = initTestDB();
    hasTaskSchedule = tableExists(db, 'task_schedule');
  ...
  describe('Step 1: 베스트셀러에서 신규 상품 조회', () => {
    it('베스트셀러 상품 목록에서 신규 상품을 필터링해야 함', () => {
      ...
      // 기존 등록 상품 추가
      db.prepare(`
        INSERT INTO coupang_product (coupang_id, product_id, product_name, deep_link, category_id)
        VALUES (?, ?, ?, ?, ?)
      `).run('existing_1', 'prod_001', '신발 A', 'https://link.coupang.com/a/xxxxx', '1001');

      // Step 1: 내목록에서 이미 등록된 상품 조회
      const rows = db.prepare('SELECT product_id FROM coupang_product').all();
    ...
    it('카테고리별로 신규 상품을 조회해야 함', () => {
      ...
      for (const [categoryId, products] of Object.entries(categoryProducts)) {
        ...
        const newProducts = products.filter(p => {
          const exists = db.prepare('SELECT 1 FROM coupang_product WHERE product_id = ?').get(p.productId);
          return !exists;
        });
  ...
  describe('Step 2: 딥링크 생성 및 내목록 등록', () => {
    it('딥링크를 생성하고 내목록에 상품을 등록해야 함', () => {
      ...
      db.prepare(`
        ...
        (coupang_id, product_id, product_name, deep_link, category_id, image_url, original_price, discount_price)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        productId,
      ...
      // 검증
      const registered = db.prepare('SELECT * FROM coupang_product WHERE coupang_id = ?').get(productId);
    ...
    it('여러 상품을 일괄 등록해야 함', () => {
      ...
      const stmt = db.prepare(`
        INSERT INTO coupang_product
        (coupang_id, product_id, product_name, deep_link, category_id, discount_price)
        VALUES (?, ?, ?, ?, ?, ?)
      `);
      ...
      // 검증
      const count = (db.prepare('SELECT COUNT(*) as cnt FROM coupang_product').get()).cnt;
  ...
  describe('Step 3: 카테고리별 자동 타이틀 생성 및 스케줄 등록', () => {
    it('상품 정보로부터 AI 타이틀을 생성해야 함', () => {
      ...
      db.prepare(`
        INSERT INTO coupang_product
        (coupang_id, product_id, product_name, deep_link, category_id)
        VALUES (?, ?, ?, ?, ?)
      `).run(uniqueId, uniqueProductId, 'Test 의류 상품', 'https://link.coupang.com/a/test_001', '1001');

      const product = db.prepare('SELECT * FROM coupang_product WHERE product_id = ?')
        .get(uniqueProductId);
    ...
    it('타이틀과 스케줄을 함께 등록해야 함', () => {
      ...
      db.prepare(`
        INSERT INTO task_schedule (id, title_id, product_url, channel, youtube_privacy, status)
        VALUES (?, ?, ?, ?, ?, ?)
      ...
      // 검증
      const savedSchedule = db.prepare('SELECT * FROM task_schedule WHERE id = ?')
        .get(scheduleId);
  ...
  describe('전체 파이프라인: 신규 상품 → 내목록 등록 → 자동 스케줄', () => {
    it('완전 자동화 파이프라인을 실행해야 함', () => {
      ...
      // 이 테스트 시작 전 count 저장
      const initialProducts = (db.prepare('SELECT COUNT(*) as cnt FROM coupang_product').get()).cnt;
      const initialTitles = (db.prepare('SELECT COUNT(*) as cnt FROM video_titles').get()).cnt;
      const initialSchedules = scheduleCount();
      ...
      const totalProducts = (db.prepare('SELECT COUNT(*) as cnt FROM coupang_product').get()).cnt;
      const totalTitles = (db.prepare('SELECT COUNT(*) as cnt FROM video_titles').get()).cnt;
      const totalSchedules = scheduleCount();
    ...
    it('중복 상품은 등록하지 않아야 함', () => {
      ...
      // 중복 체크
      const isDuplicate = db.prepare('SELECT 1 FROM coupang_product WHERE product_id = ?')
