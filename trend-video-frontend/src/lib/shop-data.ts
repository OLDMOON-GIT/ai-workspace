...
export async function fetchShopData(): Promise<{
  ...
  if (IS_VERCEL) {
    ...
    const products = await db.prepare(`
      SELECT
        id, title, description, category, thumbnail_url, deep_link,
...
export async function fetchProductById(id: string): Promise<Product | null> {
  if (IS_VERCEL) {
    ...
    return await db.prepare(`
      SELECT * FROM coupang_product
      WHERE coupang_id = ? AND status IN ('active', 'published')
...
export async function fetchAllCategories(): Promise<string[]> {
  if (IS_VERCEL) {
    ...
    const result = await db.prepare(`
      SELECT DISTINCT category FROM coupang_product
      WHERE status IN ('active', 'published')
...
export async function fetchAllProductIds(): Promise<string[]> {
  if (IS_VERCEL) {
    ...
    const result = await db.prepare(`
      SELECT coupang_id as coupangId FROM coupang_product
      WHERE status IN ('active', 'published')
    `).all() as { coupangId: string }[];
    return result.map(r => r.coupangId);
  }
}
