...
function fixJsonString(jsonString: string, log: boolean = false): string {
  ...
  // "description": "설명"(50 characters) -> "description": "설명"
  fixed = fixed.replace(/"([^"\\]*(?:\\.[^"\\]*)*)"\s*:\s*"([^"\\]*(?:\\.[^"\\]*)*)"\s*\([^)]+\)/g, '"$1": "$2"');

  // Step 0.5: title 필드의 따옴표 제거 (JSON 파싱 오류 방지)
  // ⭐ 전역 JSON 파싱 시 반드시 parseJsonSafely 사용! (title에 따옴표 등 특수문자 대응)
  // "title": ""용서해줘" 시골집에서..." -> "title": "용서해줘 시골집에서..."
  // 더 강력한 방법: "title": " 부터 다음 줄바꿈 전의 ", 까지 찾기
  const titlePattern = /"title"\s*:\s*"/g;
  let titleMatch;
  while ((titleMatch = titlePattern.exec(fixed)) !== null) {
    const startIdx = titleMatch.index + titleMatch[0].length; // 값 시작 위치
    // 값의 끝 찾기: ", 또는 "\n 또는 "} 패턴
    let endIdx = startIdx;
    ...
    if (endIdx > startIdx) {
      const titleValue = fixed.substring(startIdx, endIdx);
      // 값 내부의 모든 따옴표 제거
      const cleanValue = titleValue.replace(/"/g, '');
      fixed = fixed.substring(0, startIdx) + cleanValue + fixed.substring(endIdx);
    }
  }

  // Step 1: JSON 시작점 찾기
  const jsonStartMatch = fixed.match(/\{\s*"title"/);
  if (jsonStartMatch && jsonStartMatch.index !== undefined && jsonStartMatch.index > 0) {
    fixed = fixed.substring(jsonStartMatch.index);
  } else {
  ...
  const otherLongFields = ['imagePrompt', 'image_prompt', 'description', 'text', 'visual_description', 'prompt', 'audio_description', 'soraPrompt', 'sora_prompt'];
  ...
  const allLongFields = ['narration', 'imagePrompt', 'image_prompt', 'description', 'text', 'visual_description', 'prompt', 'audio_description', 'soraPrompt', 'sora_prompt'];
