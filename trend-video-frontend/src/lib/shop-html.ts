...
export function generateShopHtml(products: PublishedProduct[], nickname?: string): string {
  ...
  const productCards = products
    .map((product) => {
      ...
      const discountPrice = formatCurrency(product.discountPrice);
      const originalPrice = formatCurrency(product.originalPrice);

      return `
    <div class="coupang-product-card" data-category="${categoryLabel}" data-product-id="${escapeHtml(product.coupangId)}" style="background: #1e293b; border-radius: 12px; overflow: hidden; transition: transform 0.2s; position: relative; display: flex; flex-direction: column; height: 100%;">
      <button
        class="bookmark-btn"
        data-product-id="${escapeHtml(product.coupangId)}"
        onclick="window.toggleBookmark && window.toggleBookmark('${escapeHtml(product.coupangId)}'); return false;"
