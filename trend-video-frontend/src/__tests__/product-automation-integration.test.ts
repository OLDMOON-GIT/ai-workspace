...
describe('🛍️ 자동화 상품 등록 프로세스 통합 테스트', () => {
  ...
  const skipIfNoSchedule = () => {
    if (!hasTaskSchedule) {
      expect(true).toBe(true); // 스케줄 테이블이 제거된 스펙에서는 스킵
      return true;
    }
    return false;
  };
  ...
  describe('Suite 1: 내 목록 상품 딥링크 검증', () => {
    test('✅ 딥링크가 있는 상품은 내 목록에 저장됨', () => {
      ...
      const result = db.prepare(`
        SELECT * FROM coupang_product WHERE product_id = ?
    ...
    test('✅ 카테고리별 상품 조회 시 딥링크 검증', () => {
      ...
      const results = db.prepare(`
        SELECT * FROM coupang_product WHERE category_id = ? AND user_id = ?
  ...
  describe('Suite 3: 자동화 스케줄러 실행 시 URL 재검증', () => {
    test('✅ 올바른 딥링크로 스케줄 처리', () => {
      ...
      // 상태 업데이트 가능
      if (isValidDeepLink) {
        db.prepare('UPDATE task_schedule SET product_url = ? WHERE id = ?')
