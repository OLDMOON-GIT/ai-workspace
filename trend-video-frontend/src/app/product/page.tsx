...
// API 응답(camelCase) → 화면 모델(snake_case)로 정규화
const normalizeProduct = (apiProduct: ApiProduct | any): Product => {
  const id = apiProduct.id || apiProduct.coupangId || apiProduct.coupang_id || '';
  return {
    ...apiProduct,
    id,
    user_id: apiProduct.user_id ?? apiProduct.userId,
    thumbnail_url: apiProduct.thumbnail_url ?? apiProduct.thumbnailUrl ?? apiProduct.image_url ?? apiProduct.imageUrl ?? '',
    image_url: apiProduct.image_url ?? apiProduct.imageUrl ?? apiProduct.thumbnail_url ?? apiProduct.thumbnailUrl ?? '',
    product_url: apiProduct.product_url ?? apiProduct.productUrl,
    deep_link: apiProduct.deep_link ?? apiProduct.deepLink,
    original_price: apiProduct.original_price ?? apiProduct.originalPrice,
    discount_price: apiProduct.discount_price ?? apiProduct.discountPrice,
    view_count: apiProduct.view_count ?? apiProduct.viewCount ?? 0,
    click_count: apiProduct.click_count ?? apiProduct.clickCount ?? 0,
    is_favorite: apiProduct.is_favorite ?? apiProduct.isFavorite,
    queue_id: apiProduct.queue_id ?? apiProduct.queueId,
    queue_status: apiProduct.queue_status ?? apiProduct.queueStatus,
    queue_retry_count: apiProduct.queue_retry_count ?? apiProduct.queueRetryCount,
    queue_error: apiProduct.queue_error ?? apiProduct.queueError
  };
};
...
export default function CoupangProductsAdminPage() {
  ...
  // 탭 관리 - URL에서 초기값 읽기
  const [activeTab, setActiveTab] = useState<ProductTabType>(() => {
  ...
  // 쿠팡상품 서브 탭
  const [coupangSubTab, setCoupangSubTab] = useState<CoupangSubTabType>('bestseller');

  // 탭 변경 시 URL도 업데이트
  const changeTab = (tab: ProductTabType) => {
    setActiveTab(tab);
    const params = new URLSearchParams(window.location.search);
    if (tab === 'pending') {
  ...
  // 쿠팡 파트너스 통계
  const [stats, setStats] = useState<CoupangStats>({
    totalClicks: 0,
    totalLinks: 0,
    estimatedRevenue: 0,
    conversionRate: 0
  ...
  const [bestsellerResults, setBestsellerResults] = useState<BestsellerItem[]>([]);
  ...
  // 최근 검색어 쿠키 관리 (최대 4개)
  const RECENT_SEARCHES_COOKIE = 'coupang_recent_searches';
  ...
  const loadRecentSearches = (): string[] => {
    try {
      const cookieValue = document.cookie
        .split('; ')
        .find(row => row.startsWith(RECENT_SEARCHES_COOKIE + '='));
      if (cookieValue) {
  ...
  const saveRecentSearches = (searches: string[]) => {
    try {
      const value = encodeURIComponent(JSON.stringify(searches.slice(0, MAX_RECENT_SEARCHES)));
      // 30일 후 만료
      const expires = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toUTCString();
      document.cookie = `${RECENT_SEARCHES_COOKIE}=${value}; expires=${expires}; path=/`;
    } catch (e) {
  ...
  const coupangSearchCacheRef = useRef<Record<string, { data: any[]; timestamp: number }>>({});
  ...
  const checkOngoingJob = async () => {
    try {
      ...
      if (data.taskId) {
        console.log('🔄 진행 중인 작업 발견:', data.taskId);
        setCurrentJobId(data.taskId);
        setIsMoving(true);
        ...
        const interval = setInterval(async () => {
          try {
            const statusRes = await fetch(`/api/job-status?taskId=${data.taskId}`);
  ...
  const loadProducts = async () => {
    try {
      // 캐시에서 먼저 로드 (즉시 표시)
      const cached = localStorage.getItem('coupang_product_cache');
      if (cached) {
        try {
          const { products: cachedProducts } = JSON.parse(cached);
          if (cachedProducts && cachedProducts.length > 0) {
            const normalized = cachedProducts.map((p: any) => normalizeProduct(p));
            setProducts(normalized);
            const cats = Array.from(new Set(normalized.map((p: Product) => p.category)));
      ...
      if (res.ok) {
        const normalized = (data.products || []).map((p: any) => normalizeProduct(p));
        setProducts(normalized);
        ...
        // 캐시에 저장
        localStorage.setItem('coupang_product_cache', JSON.stringify({
          products: normalized,
          timestamp: Date.now()
        }));
      ...
      if (res.ok && data.products) {
        ...
        data.products.forEach((product: any) => {
          ...
          if (!categoryMap.has(category)) {
            categoryMap.set(category, { count: 0, thumbnail: product.thumbnail_url });
          }
    ...
    try {
      const res = await fetch(`/api/coupang-products/${editingProduct.id}`, {
        ...
        body: JSON.stringify({
          ...
          product_url: editForm.product_url || undefined  // ⭐ 상품 링크 (변경 시 딥링크 자동 재생성)
        })
      });
  ...
  const handleStopCrawl = async () => {
    ...
    try {
      const res = await fetch(`/api/crawl-product-links?taskId=${currentJobId}`, {
        method: 'DELETE'
      });
  ...
  const handleStopBatchMove = async () => {
    ...
    try {
      const res = await fetch(`/api/pending-products/batch?taskId=${currentJobId}`, {
        method: 'DELETE'
      });
  ...
  const pollJobStatus = async (taskId: string) => {
    try {
      const res = await fetch(`/api/crawl-product-links?taskId=${taskId}`);
      const data = await safeJsonResponse(res);
  ...
  const coupangSearchCacheExpiryMs = 10 * 60 * 1000; // 10분

  const runCoupangSearch = async (keyword: string, options?: { silent?: boolean }) => {
    const trimmed = keyword.trim();
    if (!trimmed) {
    ...
    const cacheKey = `coupang_search_${trimmed.toLowerCase()}`;
    const now = Date.now();
    ...
    try {
      // 메모리 캐시 확인
      const cachedInRef = coupangSearchCacheRef.current[cacheKey];
      if (cachedInRef && now - cachedInRef.timestamp < coupangSearchCacheExpiryMs) {
        setCoupangSearchResults(cachedInRef.data || []);
        localStorage.setItem('coupang_last_search_key', cacheKey);
        localStorage.setItem('coupang_last_search_term', trimmed);
        if (!options?.silent) {
          toast.success(`${cachedInRef.data?.length || 0}개 상품 검색 (캐시)`);
        }
        return;
      }
      ...
      if (cachedStr) {
        try {
          ...
          if (cached?.timestamp && now - cached.timestamp < coupangSearchCacheExpiryMs) {
            coupangSearchCacheRef.current[cacheKey] = { data: cached.data || [], timestamp: cached.timestamp };
            setCoupangSearchResults(cached.data || []);
            localStorage.setItem('coupang_last_search_key', cacheKey);
            localStorage.setItem('coupang_last_search_term', trimmed);
            if (!options?.silent) {
              toast.success(`${(cached.data || []).length}개 상품 검색 (캐시)`);
            }
            return;
          }
        } catch {
      ...
      console.log('📡 [쿠팡검색] 응답 상태:', response.status);

      let data: any = {};
      try {
        const text = await response.text();
        console.log('📡 [쿠팡검색] 응답 본문:', text.substring(0, 200));
        data = text ? JSON.parse(text) : {};
      } catch (parseError: any) {
        console.error('❌[쿠팡검색] JSON 파싱 실패:', parseError);
        throw new Error('서버 응답 파싱 실패');
      }

      if (response.ok) {
        const products = data.products || [];
        setCoupangSearchResults(products);
        const cachePayload = { data: products, timestamp: now };
        coupangSearchCacheRef.current[cacheKey] = cachePayload;
        ...
        if (!options?.silent) {
          toast.success(`${products.length}개 상품 검색 완료`);
        }
      } else {
        console.error('❌[쿠팡검색] 에러:', data);
        throw new Error(data.error || `검색 실패 (${response.status})`);
      }
    } catch (error: any) {
      console.error('❌[쿠팡검색] 예외:', error);
  ...
  useEffect(() => {
    ...
    try {
      const lastBestsellerKey = localStorage.getItem('bestseller_last_key');
      if (lastBestsellerKey) {
        const cachedStr = localStorage.getItem(lastBestsellerKey);
        if (cachedStr) {
          const cached = JSON.parse(cachedStr);
          const now = Date.now();
          if (cached?.timestamp && now - cached.timestamp < cacheExpiryMs && Array.isArray(cached.data)) {
            setBestsellerResults(cached.data);
            if (cached.category) {
              setBestsellerCategory(cached.category);
            }
          }
        }
      }
    ...
    try {
      const lastSearchKey = localStorage.getItem('coupang_last_search_key');
      const lastSearchTerm = localStorage.getItem('coupang_last_search_term');
      const now = Date.now();
      if (lastSearchKey) {
        const cachedStr = localStorage.getItem(lastSearchKey);
        if (cachedStr) {
          const cached = JSON.parse(cachedStr);
          if (cached?.timestamp && now - cached.timestamp < coupangSearchCacheExpiryMs) {
            coupangSearchCacheRef.current[lastSearchKey] = { data: cached.data || [], timestamp: cached.timestamp };
            setCoupangSearchResults(cached.data || []);
            if (lastSearchTerm) {
              setSearchKeyword(lastSearchTerm);
            }
          }
        }
      }
    } catch (error) {
      console.error('쿠팡 검색 캐시 로드 실패:', error);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      ...
      <div className="max-w-7xl mx-auto pb-32">
                    ...
                    onClick={async () => {
                      ...
                      for (const product of selectedProducts) {
                        try {
                          ...
                          // localStorage에서 미디어 모드 가져오기 (automation 페이지에서 선택한 값)
                          const savedMediaMode = typeof window !== 'undefined'
                            ? localStorage.getItem('automation_selected_media_mode')
                            : null;
                          ...
                          const response = await fetch('/api/automation/titles', {
                            ...
                            body: JSON.stringify({
                              ...
                              promptFormat: 'product',  // type → promptFormat
                              ...
                              mediaMode: savedMediaMode || 'crawl',  // ⭐ localStorage 또는 기본값 'crawl'
                              aiModel: 'gpt-4o',       // model → aiModel
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredProducts.slice(0, myListDisplayCount).map((product, idx) => (
            <div
              key={product.id || product.product_url || `product-${idx}`}
              ...
              <div className="p-4">
                ...
                <div className="bg-slate-900/50 rounded-lg p-2 mb-3 flex items-center justify-between gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-blue-400 hover:text-blue-300 font-mono break-all line-clamp-1 flex-1 underline"
                  >
                    {product.deep_link}
                  </a>
                  <button
        ...
        {products.length === 0 && (
          <div className="rounded-2xl border-2 border-dashed border-slate-600 p-12 text-center">
            ...
            <p className="text-xl text-slate-300 mb-2">등록된 상품이 없습니다</p>
            <p className="text-slate-500 mb-6">상품을 추가하여 쇼핑몰을 시작해보세요!</p>
            <div className="flex justify-center gap-4">
              <button
                onClick={() => setIsSidebarOpen(true)}
                className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 rounded-lg font-semibold text-white hover:from-purple-500 hover:to-pink-500 transition"
              >
                ➕ 딥링크로 상품 추가
              </button>
              <button
                onClick={() => changeTab('coupang')}
                className="px-6 py-3 bg-emerald-600 rounded-lg font-semibold text-white hover:bg-emerald-500 transition"
              >
                🏆 베스트셀러에서 추가
              </button>
            </div>
        ...
        {activeTab === 'pending' && (
          <>
            ...
            {pendingProducts.length === 0 && (
              <div className="rounded-2xl border-2 border-dashed border-slate-600 p-12 text-center">
                <div className="text-6xl mb-4">⏳</div>
                <p className="text-xl text-slate-300 mb-2">대기 중인 상품이 없습니다</p>
                <p className="text-slate-500 mb-4">링크 모음을 크롤링하면 상품들이 여기에 표시됩니다</p>
                <div className="flex justify-center gap-2 text-sm text-slate-400">
                  <span className="px-3 py-1 bg-slate-700 rounded-full">1. 링크모음 URL 입력</span>
                  <span>→</span>
                  <span className="px-3 py-1 bg-slate-700 rounded-full">2. 크롤링 시작</span>
                  <span>→</span>
                  <span className="px-3 py-1 bg-emerald-600/30 text-emerald-300 rounded-full">3. 내 목록으로 이동</span>
                </div>
              </div>
            )}
          </>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            ...
            {coupangSubTab === 'bestseller' && (
              <>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (cached) {
                          try {
                            const { data, timestamp, category } = JSON.parse(cached);
                            const now = Date.now();
                            if (now - timestamp < cacheExpiry) {
                              console.log('💾 [캐시] 베스트셀러 캐시 사용:', Math.floor((cacheExpiry - (now - timestamp)) / 1000 / 60), '분 남음');
                              setBestsellerResults(data);
                              if (category) {
                                setBestsellerCategory(category);
                              }
                              localStorage.setItem('bestseller_last_key', cacheKey);
                ...
                {/* 빈 상태 안내 */}
                {bestsellerResults.length === 0 && !isFetchingBestseller && (
                  <div className="rounded-2xl border-2 border-dashed border-slate-600 p-8 text-center">
                    <div className="text-6xl mb-4">🏆</div>
                    <p className="text-lg text-slate-300 mb-2">베스트셀러 상품이 없습니다</p>
                    <p className="text-sm text-slate-500 mb-4">
                      위의 카테고리를 선택하고 <strong className="text-emerald-400">가져오기</strong> 버튼을 눌러주세요
                    </p>
                    <div className="flex justify-center gap-2">
                      <span className="px-3 py-1 bg-slate-700 rounded-full text-xs text-slate-400">📱 전자제품</span>
                      <span className="px-3 py-1 bg-slate-700 rounded-full text-xs text-slate-400">👗 패션</span>
                      <span className="px-3 py-1 bg-slate-700 rounded-full text-xs text-slate-400">💄 뷰티</span>
                    </div>
                  </div>
                )}
                  ...
                  <input
                    ...
                    onKeyDown={async (e) => {
                      if (e.key === 'Enter' && searchKeyword.trim()) {
                        await runCoupangSearch(searchKeyword);
                      }
                    }}
...
disabled={isCoupangSearching}
                ...
                {recentSearches.length > 0 && (
                  <div className="mb-4 flex flex-wrap items-center gap-2">
                    <span className="text-sm text-slate-400">최근 검색:</span>
                    {recentSearches.map((keyword) => (
                      <div
                        key={keyword}
                        className="group flex items-center gap-1 rounded-full bg-slate-700/50 px-3 py-1 text-sm text-slate-300 hover:bg-slate-600/50 transition"
                      >
                        <button
                          onClick={() => {
                            setSearchKeyword(keyword);
                            runCoupangSearch(keyword);
                          }}
                ...
                {/* 빈 상태 안내 */}
                {coupangSearchResults.length === 0 && !isCoupangSearching && (
                  <div className="rounded-2xl border-2 border-dashed border-slate-600 p-8 text-center">
                    <div className="text-6xl mb-4">🔍</div>
                    <p className="text-lg text-slate-300 mb-2">검색 결과가 없습니다</p>
                    <p className="text-sm text-slate-500 mb-4">
                      위의 검색창에 상품명을 입력하고 <strong className="text-blue-400">검색</strong> 버튼을 눌러주세요
                    </p>
                    <div className="flex justify-center gap-2">
                      <span className="px-3 py-1 bg-slate-700 rounded-full text-xs text-slate-400">노트북</span>
                      <span className="px-3 py-1 bg-slate-700 rounded-full text-xs text-slate-400">이어폰</span>
                      <span className="px-3 py-1 bg-slate-700 rounded-full text-xs text-slate-400">청소기</span>
                    </div>
                  </div>
                )}
          ...
          >
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-300 mb-2">
                  🔗 상품 링크
                  <span className="text-xs text-slate-400 ml-2">(품절 시 새 링크로 교체 - 딥링크 자동 재생성)</span>
                </label>
                <input
                  type="url"
                ...
                />
                {editingProduct?.deep_link && (
                  <p className="mt-1 text-xs text-slate-400">
                    현재 딥링크: <a href={editingProduct.deep_link} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">{editingProduct.deep_link}</a>
                  </p>
                )}
              </div>
