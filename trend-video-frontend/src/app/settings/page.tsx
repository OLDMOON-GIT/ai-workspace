...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'youtube' | 'google-sites' | 'password' | 'profile' | 'coupang' | 'social-media' | 'category'>('profile');
  ...
  const getChannelEmbedUrl = (channelId: string) => {
    if (!channelId) return '';
    // YouTube uploads playlist ID = "UU" + channelId without the leading "UC"
    if (channelId.startsWith('UC')) {
      return `https://www.youtube.com/embed/videoseries?list=UU${channelId.slice(2)}`;
    }
    return `https://www.youtube.com/embed/videoseries?list=${channelId}`;
  };

  // URL 파라미터에서 탭 읽기 (URL 변경 시마다 업데이트)
  useEffect(() => {
    const tab = searchParams.get('tab');
    if (tab === 'youtube' || tab === 'google-sites' || tab === 'password' || tab === 'profile' || tab === 'coupang' || tab === 'social-media' || tab === 'category') {
      setActiveTab(tab as any);
    } else if (!tab) {
      // 탭 파라미터가 없으면 기본값으로 profile 설정
      setActiveTab('profile');
    }
  }, [searchParams]);

  // 초기 데이터 로드 및 success 파라미터 처리
  useEffect(() => {
  ...
  const handleAddCategory = async () => {
    if (!newCategory.category || !newCategory.displayName) {
      toast.error('카테고리명과 표시 이름은 필수입니다.');
      return;
    }
    ...
    try {
      const keywords = newCategory.keywords
        ? newCategory.keywords.split(',').map(k => k.trim()).filter(k => k)
        : [];
      ...
      if (response.ok) {
        ...
        setNewCategory({ category: '', displayName: '', keywords: '', description: '' });
  ...
  const handleUpdateCategory = async () => {
    ...
    setIsSavingCategory(true);
    try {
      const keywords = Array.isArray(editingCategory.keywords)
        ? editingCategory.keywords
        : editingCategory.keywords?.split(',').map((k: string) => k.trim()).filter((k: string) => k) || [];
      ...
      const response = await fetch('/api/user/categories', {
        ...
        body: JSON.stringify({
          ...
          displayName: editingCategory.display_name,
          keywords,
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    ...
    try {
      const response = await fetch(`/api/user/categories?id=${categoryId}`, {
        method: 'DELETE',
        credentials: 'include'
      });
  ...
  const handleAddChannel = async () => {
    ...
    try {
      ...
      // API가 직접 리다이렉트하므로 window.location.href로 이동
  ...
  const handleReauthorize = async (channelId: string) => {
    ...
    try {
      ...
      // API가 직접 리다이렉트하므로 window.location.href로 이동
      window.location.href = `/api/youtube/oauth-start?reauth=true&channelId=${channelId}`;
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex gap-2 flex-wrap">
          ...
          <button
            onClick={() => router.push('/settings?tab=youtube')}
            className={`px-6 py-3 rounded-lg text-lg font-semibold transition ${
              activeTab === 'youtube'
          ...
          <button
            onClick={() => router.push('/settings?tab=coupang')}
            className={`px-6 py-3 rounded-lg text-lg font-semibold transition ${
              activeTab === 'coupang'
          ...
          <button
            onClick={() => router.push('/settings?tab=social-media')}
            className={`px-6 py-3 rounded-lg text-lg font-semibold transition ${
          ...
          <button
            onClick={() => router.push('/settings?tab=category')}
            className={`px-6 py-3 rounded-lg text-lg font-semibold transition ${
              activeTab === 'category'
          ...
          <div className="rounded-2xl border border-slate-600 bg-slate-800/50 backdrop-blur">
            ...
            <div className="p-8 border-b border-slate-700">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white">YouTube 채널 관리 ({channels.length}개)</h2>
                <button
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex gap-2">
                          <a
                            ...
                            className="px-3 py-2 bg-red-500 hover:bg-red-600 text-white text-sm font-semibold rounded-lg transition flex items-center gap-1"
                          >
                          ...
                          <button
                            ...
                            className="px-3 py-2 bg-yellow-600 hover:bg-yellow-700 disabled:bg-slate-600 text-white text-sm font-semibold rounded-lg transition flex items-center gap-1"
                          >
            ...
            <div className="p-8 border-t border-slate-700">
              <h3 className="text-lg font-semibold text-white mb-3">채널별 콘텐츠 미리보기</h3>
              <p className="text-sm text-slate-400 mb-4">
                각 채널의 업로드 플레이리스트를 iframe으로 바로 확인할 수 있습니다. (일부 채널은 임베드가 제한될 수 있습니다)
              </p>
              <div className="grid gap-6 md:grid-cols-2">
                {channels.map((channel) => {
                  const embedUrl = getChannelEmbedUrl(channel.channelId);
                  return (
                    <div
                      key={channel.id}
                      className="rounded-xl border border-slate-700 bg-slate-900/50 hover:border-slate-600 transition p-4 space-y-3"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-white font-semibold">{channel.channelTitle}</p>
                            <p className="text-xs text-slate-400">업로드 플레이리스트</p>
                          </div>
                        </div>
                        <a
                          href={`https://www.youtube.com/channel/${channel.channelId}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="px-3 py-1.5 text-xs font-semibold rounded bg-red-600 hover:bg-red-700 text-white transition"
                        >
                          채널로 이동
                        </a>
                      </div>
                      {embedUrl ? (
                        <div className="aspect-video rounded-lg overflow-hidden border border-slate-700 bg-black">
                          <iframe
                            key={embedUrl}
                            src={embedUrl}
                            title={`${channel.channelTitle} 업로드`}
                        ...
                        <p className="text-sm text-slate-400">채널 ID를 찾을 수 없어 미리보기를 표시할 수 없습니다.</p>
                      )}
                    </div>
                  );
                })}
        ...
        {activeTab === 'category' && (
          <div className="rounded-2xl border border-slate-600 bg-slate-800/50 p-8 backdrop-blur">
            <h2 className="text-2xl font-bold text-white mb-4">🏷️ 카테고리 관리</h2>
            <p className="text-slate-400 mb-6 text-sm">
              영상 생성 시 사용할 카테고리를 관리합니다. 각 카테고리별로 키워드를 설정하여 자동화에 활용할 수 있습니다.
            </p>

            {/* 기존 카테고리 목록 */}
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-white mb-4">📋 내 카테고리</h3>
              <div className="space-y-3">
                {categories.length === 0 ? (
                  ...
                  categories.map((cat) => (
                    <div
                      key={cat.id}
                      className="p-4 rounded-lg border bg-slate-900/50 border-slate-700 hover:border-slate-600 transition"
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <h4 className="text-white font-semibold">{cat.display_name}</h4>
            ...
            {!editingCategory && (
              <div className="border-t border-slate-700 pt-6">
                <h3 className="text-lg font-semibold text-white mb-4">➕ 새 카테고리 추가</h3>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="예: 북한탈북자사연"
                        className="w-full rounded-lg bg-slate-900 border border-slate-600 px-4 py-2 text-white placeholder-slate-400 focus:border-purple-500 focus:outline-none"
                      />
                      <p className="mt-1 text-xs text-slate-500">
                        영문, 한글, 숫자만 사용 (공백 없이)
                      </p>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="예: 탈북자 사연, 북한 실화, 탈북 이야기"
                      className="w-full rounded-lg bg-slate-900 border border-slate-600 px-4 py-2 text-white placeholder-slate-400 focus:border-purple-500 focus:outline-none"
                    />
                    <p className="mt-1 text-xs text-slate-500">
                      영상 소재 검색에 사용될 키워드들
            ...
            {editingCategory && (
              <div className="border-t border-slate-700 pt-6">
                <h3 className="text-lg font-semibold text-white mb-4">✏️ 카테고리 수정</h3>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-300 mb-2">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={Array.isArray(editingCategory.keywords) ? editingCategory.keywords.join(', ') : editingCategory.keywords}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={isSavingCategory}
                      className="flex-1 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-3 text-white font-bold hover:from-blue-500 hover:to-purple-500 transition disabled:opacity-50"
                    >
                      {isSavingCategory ? '저장 중...' : '💾 수정 저장'}
                    </button>
                    <button
            ...
            <div className="mt-6 bg-blue-950/30 border border-blue-500/20 rounded-lg p-4">
              <h3 className="text-sm font-semibold text-blue-300 mb-2">💡 카테고리 활용 방법</h3>
              <div className="space-y-1 text-xs text-slate-400">
                <p>• <strong className="text-white">기본 카테고리:</strong> 상품, 시니어사연은 시스템 기본 카테고리로 삭제/수정할 수 없습니다.</p>
                <p>• <strong className="text-white">키워드 활용:</strong> 설정한 키워드는 자동화 시스템에서 영상 소재 검색에 사용됩니다.</p>
                <p>• <strong className="text-white">자동화 연동:</strong> 영상 생성 자동화 시 카테고리를 선택하여 관련 콘텐츠를 자동 생성합니다.</p>
              </div>
            </div>
          </div>
        )}
