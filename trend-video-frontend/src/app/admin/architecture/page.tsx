...
type TabType = 'architecture' | 'erd' | 'api' | 'sequence';
...
function ArchitectureContent() {
    ...
    const renderMermaid = () => {
      setTimeout(() => {
        ...
        if (mermaidNodes.length > 0) {
          // Mermaid v11+ requires await and catch for proper error handling
          mermaid.run({
            nodes: mermaidNodes,
          }).catch((error) => {
  ...
  const handleTabChange = (tab: TabType) => {
    ...
    if (tab === 'architecture') {
      ...
      router.push(`/admin/architecture?tab=${tab}`);
    }
  };
          ...
          <button
            ...
            className={`flex-1 rounded-xl border py-4 px-6 text-lg font-semibold transition ${
              activeTab === 'api'
                ...
                : 'border-white/10 bg-slate-800/50 text-slate-400 hover:border-green-500/50 hover:text-white'
            }`}
          >
            📋 API 명세서
          </button>
          <button
            ...
            className={`flex-1 rounded-xl border py-4 px-6 text-lg font-semibold transition ${
              activeTab === 'sequence'
                ...
                : 'border-white/10 bg-slate-800/50 text-slate-400 hover:border-orange-500/50 hover:text-white'
            }`}
          >
            🔄 시퀀스 다이어그램
          </button>
        ...
        <div className="mb-8 rounded-2xl border border-white/10 bg-slate-800/50 p-8 backdrop-blur">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-6">
              ...
              <div className="space-y-2 text-sm text-slate-300">
                <p>• <code className="rounded bg-slate-700 px-2 py-1">user</code> - 사용자 정보, 크레딧</p>
                <p>• <code className="rounded bg-slate-700 px-2 py-1">content</code> - 대본/영상 컨텐츠 (통합)</p>
                <p>• <code className="rounded bg-slate-700 px-2 py-1">task</code> - 자동화 작업 (v3)</p>
                <p>• <code className="rounded bg-slate-700 px-2 py-1">task_schedule</code> - 예약 스케줄</p>
                <p>• <code className="rounded bg-slate-700 px-2 py-1">task_queue</code> - 작업 큐</p>
                <p>• <code className="rounded bg-slate-700 px-2 py-1">user_charge_request</code> - 충전 요청</p>
              </div>
            </div>
          </div>
        </div>
...
{`erDiagram
    ...
    USERS ||--o{ USER_CREDIT_HISTORY : "has"
    ...
    CONTENTS ||--o{ CONTENTS : "converts-to"`}
            ...
            <div className="space-y-6">
                ...
                {/* USER_CREDIT_HISTORY 테이블 */}
                <div
                  className="rounded-xl border border-yellow-500/30 bg-slate-800/50 p-6 backdrop-blur"
                >
                  <h3 className="mb-4 text-2xl font-bold text-yellow-300">💰 USER_CREDIT_HISTORY (크레딧 내역)</h3>
                  <div className="table-content overflow-x-auto">
                    <table className="w-full border-collapse text-sm">
                      <thead>
              ...
              <div className="rounded-xl border border-blue-500/30 bg-blue-500/10 p-6">
                ...
                <div className="space-y-4 text-sm text-slate-300">
                  ...
                  <div>
                    <h4 className="font-semibold text-white">💰 USER_CREDIT_HISTORY</h4>
              ...
              <div className="rounded-xl border border-green-500/30 bg-green-500/10 p-6">
                ...
                <div className="space-y-2 text-sm text-slate-300">
                  ...
                  <p>• USERS → USER_CREDIT_HISTORY: 1:N (크레딧 변동 내역)</p>
              ...
              <div className="rounded-xl border border-purple-500/30 bg-purple-500/10 p-6">
                ...
                <div className="space-y-4 text-sm text-slate-300">
                  <div>
                    ...
                    <p>• USERS, CONTENTS, USER_CREDIT_HISTORY 등 대부분의 테이블은 UUID 기반 id 필드</p>
                  ...
                  <div>
                    ...
                    <p>• ON DELETE CASCADE: USERS 삭제 시 관련 CONTENTS, USER_CREDIT_HISTORY 등 모두 삭제</p>
                  ...
                  <div>
                    ...
                    <p>• USER_CREDIT_HISTORY(userId, createdAt): 크레딧 내역 조회</p>
              ...
              <div className="rounded-xl border border-orange-500/30 bg-orange-500/10 p-6">
                ...
                <div className="space-y-4 text-sm text-slate-300">
                  ...
                  <div>
                    ...
                    <p className="ml-4 text-slate-400">→ USER_CREDIT_HISTORY에 CHARGE 타입 기록 (balanceAfter 업데이트)</p>
                  </div>
                  <div>
                    ...
                    <p className="ml-4 text-slate-400">→ USER_CREDIT_HISTORY에 USE 타입 기록</p>
              ...
              <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-6">
                ...
                <div className="space-y-3 text-sm text-slate-300">
                  ...
                  <div>
                    ...
                    <p className="mb-1">2. USER_CREDIT_HISTORY에 REFUND 타입 레코드 생성</p>
        ...
        {activeTab === 'api' && (
          <div data-tab="api">
            {/* API 개요 */}
            <div className="mb-8 rounded-2xl border border-green-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h2 className="mb-6 text-2xl font-bold text-white">📋 API 명세서</h2>

              {/* 통계 카드 */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div className="rounded-xl border border-green-500/30 bg-slate-900/50 p-4 text-center">
                  <div className="text-3xl font-bold text-green-400">167</div>
                  <div className="text-sm text-slate-400">총 API 엔드포인트</div>
                </div>
                <div className="rounded-xl border border-blue-500/30 bg-slate-900/50 p-4 text-center">
                  <div className="text-3xl font-bold text-blue-400">145</div>
                  <div className="text-sm text-slate-400">GET 요청</div>
                </div>
                <div className="rounded-xl border border-purple-500/30 bg-slate-900/50 p-4 text-center">
                  <div className="text-3xl font-bold text-purple-400">118</div>
                  <div className="text-sm text-slate-400">POST 요청</div>
                </div>
                <div className="rounded-xl border border-red-500/30 bg-slate-900/50 p-4 text-center">
                  <div className="text-3xl font-bold text-red-400">33</div>
                  <div className="text-sm text-slate-400">DELETE 요청</div>
                </div>
              </div>

              {/* 인증 설명 */}
              <div className="mb-6 rounded-xl border border-yellow-500/30 bg-yellow-500/10 p-4">
                <h3 className="text-lg font-bold text-yellow-300 mb-2">🔐 인증 방식</h3>
                <p className="text-slate-300">모든 API는 세션 쿠키 기반 인증을 사용합니다. <code className="bg-slate-700 px-2 py-1 rounded text-green-400">credentials: 'include'</code>를 반드시 포함해야 합니다.</p>
              </div>
            </div>

            {/* 인증 API */}
            <div className="mb-8 rounded-2xl border border-green-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-green-300">🔑 1. 인증 API</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b-2 border-green-500/30">
                      <th className="p-3 text-left font-bold text-green-200 bg-slate-900/50">메서드</th>
                      <th className="p-3 text-left font-bold text-green-200 bg-slate-900/50">엔드포인트</th>
                      <th className="p-3 text-left font-bold text-green-200 bg-slate-900/50">인증</th>
                      <th className="p-3 text-left font-bold text-green-200 bg-slate-900/50">설명</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-300">
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-green-400">/api/auth/login</td>
                      <td className="p-3"><span className="rounded bg-slate-600 px-2 py-0.5 text-xs">공개</span></td>
                      <td className="p-3">로그인 (email, password, rememberMe)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-green-400">/api/auth/signup</td>
                      <td className="p-3"><span className="rounded bg-slate-600 px-2 py-0.5 text-xs">공개</span></td>
                      <td className="p-3">회원가입 (email, password, name, phone, address)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-green-400">/api/auth/logout</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">로그아웃</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-green-400">/api/auth/session</td>
                      <td className="p-3"><span className="rounded bg-slate-600 px-2 py-0.5 text-xs">선택</span></td>
                      <td className="p-3">현재 세션 확인</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-green-400">/api/auth/verify-email</td>
                      <td className="p-3"><span className="rounded bg-slate-600 px-2 py-0.5 text-xs">공개</span></td>
                      <td className="p-3">이메일 인증 (token 쿼리)</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* 콘텐츠 생성 API */}
            <div className="mb-8 rounded-2xl border border-purple-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-purple-300">✨ 2. 콘텐츠 생성 API</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b-2 border-purple-500/30">
                      <th className="p-3 text-left font-bold text-purple-200 bg-slate-900/50">메서드</th>
                      <th className="p-3 text-left font-bold text-purple-200 bg-slate-900/50">엔드포인트</th>
                      <th className="p-3 text-left font-bold text-purple-200 bg-slate-900/50">인증</th>
                      <th className="p-3 text-left font-bold text-purple-200 bg-slate-900/50">설명</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-300">
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-purple-400">/api/generate-script</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">AI 스크립트 생성 (prompt, format, model)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-purple-400">/api/my-scripts</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">내 스크립트 목록 조회</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-purple-400">/api/my-videos</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">내 영상 목록 조회</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs font-bold">DELETE</span></td>
                      <td className="p-3 font-mono text-purple-400">/api/my-videos</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">영상 삭제 (taskId 쿼리)</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* 자동화 API */}
            <div className="mb-8 rounded-2xl border border-orange-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-orange-300">⚙️ 3. 자동화 API</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b-2 border-orange-500/30">
                      <th className="p-3 text-left font-bold text-orange-200 bg-slate-900/50">메서드</th>
                      <th className="p-3 text-left font-bold text-orange-200 bg-slate-900/50">엔드포인트</th>
                      <th className="p-3 text-left font-bold text-orange-200 bg-slate-900/50">인증</th>
                      <th className="p-3 text-left font-bold text-orange-200 bg-slate-900/50">설명</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-300">
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-orange-400">/api/automation/schedules</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">자동화 일정 목록</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-orange-400">/api/automation/schedules</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">자동화 일정 생성 (titleId, scheduledTime)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs font-bold">PATCH</span></td>
                      <td className="p-3 font-mono text-orange-400">/api/automation/schedules</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">자동화 일정 수정</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs font-bold">DELETE</span></td>
                      <td className="p-3 font-mono text-orange-400">/api/automation/schedules</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">자동화 일정 삭제</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-orange-400">/api/automation/logs</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">자동화 로그 조회 (titleId 쿼리)</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* 쿠팡 API */}
            <div className="mb-8 rounded-2xl border border-red-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-red-300">🛒 4. 쿠팡 API</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b-2 border-red-500/30">
                      <th className="p-3 text-left font-bold text-red-200 bg-slate-900/50">메서드</th>
                      <th className="p-3 text-left font-bold text-red-200 bg-slate-900/50">엔드포인트</th>
                      <th className="p-3 text-left font-bold text-red-200 bg-slate-900/50">인증</th>
                      <th className="p-3 text-left font-bold text-red-200 bg-slate-900/50">설명</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-300">
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-red-400">/api/coupang/products</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">베스트셀러 상품 조회 (24시간 캐싱)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-red-400">/api/coupang-products</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">상품 크롤링 요청 (productUrl)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-red-400">/api/coupang/search</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">상품 검색 (keyword)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-red-400">/api/coupang/deeplink</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">딥링크 생성 (productUrl)</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* YouTube API */}
            <div className="mb-8 rounded-2xl border border-red-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-red-300">📺 5. YouTube API</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b-2 border-red-500/30">
                  ...
                  <tbody className="text-slate-300">
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-red-400">/api/youtube/channels</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">연결된 채널 목록</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-red-400">/api/youtube/upload</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">영상 업로드 (videoPath, title, description)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-red-400">/api/youtube/oauth-start</td>
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs">필수</span></td>
                      <td className="p-3">OAuth 인증 시작</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* 관리자 API */}
            <div className="mb-8 rounded-2xl border border-yellow-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-yellow-300">👑 6. 관리자 API</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b-2 border-yellow-500/30">
                      <th className="p-3 text-left font-bold text-yellow-200 bg-slate-900/50">메서드</th>
                      <th className="p-3 text-left font-bold text-yellow-200 bg-slate-900/50">엔드포인트</th>
                      <th className="p-3 text-left font-bold text-yellow-200 bg-slate-900/50">인증</th>
                      <th className="p-3 text-left font-bold text-yellow-200 bg-slate-900/50">설명</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-300">
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-yellow-400">/api/admin/users</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">사용자 목록 조회</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-yellow-600 px-2 py-0.5 text-xs font-bold">POST</span></td>
                      <td className="p-3 font-mono text-yellow-400">/api/admin/credits</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">크레딧 추가 (userId, amount)</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-green-600 px-2 py-0.5 text-xs font-bold">GET</span></td>
                      <td className="p-3 font-mono text-yellow-400">/api/admin/prompts</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">프롬프트 목록 조회</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3"><span className="rounded bg-blue-600 px-2 py-0.5 text-xs font-bold">PUT</span></td>
                      <td className="p-3 font-mono text-yellow-400">/api/admin/prompts</td>
                      <td className="p-3"><span className="rounded bg-red-600 px-2 py-0.5 text-xs">관리자</span></td>
                      <td className="p-3">프롬프트 수정</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            {/* AI 모델 요금 */}
            <div className="rounded-2xl border border-cyan-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-cyan-300">💰 AI 모델 요금</h3>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b-2 border-cyan-500/30">
                      <th className="p-3 text-left font-bold text-cyan-200 bg-slate-900/50">모델</th>
                      <th className="p-3 text-left font-bold text-cyan-200 bg-slate-900/50">입력 (MTok)</th>
                      <th className="p-3 text-left font-bold text-cyan-200 bg-slate-900/50">출력 (MTok)</th>
                    </tr>
                  </thead>
                  <tbody className="text-slate-300">
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3 font-bold text-purple-400">Claude Sonnet 4.5</td>
                      <td className="p-3">$3.00</td>
                      <td className="p-3">$15.00</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3 font-bold text-green-400">GPT-4o</td>
                      <td className="p-3">$2.50</td>
                      <td className="p-3">$10.00</td>
                    </tr>
                    <tr className="border-t border-slate-700 hover:bg-slate-700/30">
                      <td className="p-3 font-bold text-blue-400">Gemini 2.0 Flash</td>
                      <td className="p-3 text-green-400">무료</td>
                      <td className="p-3 text-green-400">무료</td>
                    </tr>
                  </tbody>
                </table>
        ...
        {/* 시퀀스 다이어그램 탭 */}
        {activeTab === 'sequence' && (
          <div data-tab="sequence">
            {/* 로그인 시퀀스 */}
            <div className="mb-8 rounded-2xl border border-orange-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h2 className="mb-6 text-2xl font-bold text-white">🔄 시퀀스 다이어그램</h2>

              <h3 className="mb-4 text-xl font-bold text-orange-300">1. 로그인 플로우</h3>
              <div
                className="rounded-lg border border-orange-500/20 bg-slate-900/50 p-6 overflow-x-auto relative group cursor-pointer hover:border-orange-500/50 transition-all"
                onClick={(e) => {
                  const svg = e.currentTarget.querySelector('svg');
                  if (svg) {
                    handleDiagramClick(svg as SVGElement);
              ...
              >
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg bg-orange-600 px-3 py-1 text-xs text-white shadow-lg pointer-events-none">
                  🔍 클릭하여 확대
                </div>
                <pre className="language-mermaid mermaid bg-transparent border-none p-0">
{`sequenceDiagram
    ...
    alt Match
        ...
        F-->>U: Error
            ...
            {/* 스크립트 생성 시퀀스 */}
            <div className="mb-8 rounded-2xl border border-purple-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-purple-300">2. AI 스크립트 생성 플로우</h3>
              <div
                className="rounded-lg border border-purple-500/20 bg-slate-900/50 p-6 overflow-x-auto relative group cursor-pointer hover:border-purple-500/50 transition-all"
                onClick={(e) => {
                  const svg = e.currentTarget.querySelector('svg');
...
{`sequenceDiagram
    ...
    U->>F: Request Script
    F->>API: POST /api/generate-script
    Note over API: prompt format model
    API->>DB: INSERT pending
    API-->>F: scriptId
    F-->>U: Loading

    Note over API: Background
    API->>API: Load Template
    ...
    F-->>U: Display`}
            ...
            {/* 비디오 생성 시퀀스 */}
            <div className="mb-8 rounded-2xl border border-green-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-green-300">3. 비디오 렌더링 플로우</h3>
              <div
                className="rounded-lg border border-green-500/20 bg-slate-900/50 p-6 overflow-x-auto relative group cursor-pointer hover:border-green-500/50 transition-all"
                onClick={(e) => {
                  const svg = e.currentTarget.querySelector('svg');
                  if (svg) {
                    handleDiagramClick(svg as SVGElement);
              ...
              >
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg bg-green-600 px-3 py-1 text-xs text-white shadow-lg pointer-events-none">
                  🔍 클릭하여 확대
                </div>
                <pre className="language-mermaid mermaid bg-transparent border-none p-0">
{`sequenceDiagram
    ...
    API->>B: Generate Video
    Note over B: story.json images
    ...
    B-->>API: videoPath`}
            ...
            {/* 자동화 파이프라인 */}
            <div className="mb-8 rounded-2xl border border-cyan-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-cyan-300">4. 자동화 파이프라인</h3>
              <div
                className="rounded-lg border border-cyan-500/20 bg-slate-900/50 p-6 overflow-x-auto relative group cursor-pointer hover:border-cyan-500/50 transition-all"
                onClick={(e) => {
                  const svg = e.currentTarget.querySelector('svg');
                  if (svg) {
                    handleDiagramClick(svg as SVGElement);
              ...
              >
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg bg-cyan-600 px-3 py-1 text-xs text-white shadow-lg pointer-events-none">
                  🔍 클릭하여 확대
                </div>
                <pre className="language-mermaid mermaid bg-transparent border-none p-0">
{`sequenceDiagram
    ...
    participant API as API Route
    participant ADB as Automation DB
    participant W as Worker
    participant B as Backend

    A->>API: POST schedules
    API->>ADB: INSERT schedule
    API-->>A: scheduleId

    Note over W: Periodic Check
    W->>ADB: SELECT due
    ADB-->>W: Schedules

    loop Each Schedule
        W->>ADB: UPDATE running
        W->>B: Generate Script
        B-->>W: script.json
        W->>B: Generate Images
        B-->>W: images
        ...
        W->>ADB: UPDATE completed
            ...
            {/* YouTube 업로드 */}
            <div className="rounded-2xl border border-red-500/30 bg-slate-800/50 p-8 backdrop-blur">
              <h3 className="mb-4 text-xl font-bold text-red-300">5. YouTube 업로드 플로우</h3>
              <div
                className="rounded-lg border border-red-500/20 bg-slate-900/50 p-6 overflow-x-auto relative group cursor-pointer hover:border-red-500/50 transition-all"
                onClick={(e) => {
                  const svg = e.currentTarget.querySelector('svg');
                  if (svg) {
                    handleDiagramClick(svg as SVGElement);
              ...
              >
                <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg bg-red-600 px-3 py-1 text-xs text-white shadow-lg pointer-events-none">
                  🔍 클릭하여 확대
                </div>
                <pre className="language-mermaid mermaid bg-transparent border-none p-0">
{`sequenceDiagram
    ...
    U->>API: POST upload
    API->>DB: SELECT channels
    DB-->>API: Channel Token

    API->>B: youtube_upload_cli
