...
export default function TitlePoolPage() {
  ...
  // 전체처리: 모든 카테고리에 대해 제목 일괄 생성
  const handleGenerateAll = async () => {
    if (!confirm('모든 카테고리에 대해 제목을 일괄 생성하시겠습니까?\n(카테고리당 약 10개씩 생성, 90점 이상만 저장)')) {
      return;
    }

    setIsGenerating(true);
    setGenerateLog(['🚀 전체처리 시작...']);
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.success && data.titles?.length > 0) {
            totalGenerated += data.savedCount || data.titles.length;
            setGenerateLog(prev => [
              ...prev,
              `  ✅ ${data.titles.length}개 생성, ${data.savedCount || 0}개 저장 (시도: ${data.attempts})`,
              ...data.titles.slice(0, 3).map((t: any) => `    [${t.score}점] ${t.title.substring(0, 40)}...`)
            ]);
          } else {
            setGenerateLog(prev => [...prev, `  ⚠️ 90점 이상 제목 없음 (시도: ${data.attempts || 0})`]);
          }
        } else {
          setGenerateLog(prev => [...prev, `  ❌ 생성 실패`]);
        }
      } catch (error: any) {
        setGenerateLog(prev => [...prev, `  ❌ 에러: ${error.message}`]);
      }
    }

    setGenerateLog(prev => [...prev, `\n🎉 완료! 총 ${totalGenerated}개 제목 생성`]);
    setIsGenerating(false);

    // 데이터 새로고침
    loadData();
    toast.success(`${totalGenerated}개 제목 생성 완료`);
  };
  ...
  // 전체 되돌리기: 모든 사용된 제목을 미사용으로 변경
  const handleResetAllUsed = async () => {
    if (!confirm('모든 "사용됨" 제목을 "미사용"으로 되돌리시겠습니까?')) {
      return;
    }

    try {
      const res = await fetch('/api/admin/title-pool/reset-all', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ category: selectedCategory === 'all' ? null : selectedCategory })
      });

      if (res.ok) {
        const data = await res.json();
        toast.success(`${data.count || 0}개 제목 되돌리기 완료`);
        loadData();
      } else {
        toast.error('전체 되돌리기 실패');
      }
    } catch (error) {
      console.error('전체 되돌리기 실패:', error);
      toast.error('전체 되돌리기 실패');
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-400 mt-2">90점 이상 고품질 제목 모음 (패턴 기반 생성)</p>
          </div>
          {isAdmin && (
            <div className="flex gap-3">
              <button
                onClick={handleResetAllUsed}
                className="px-4 py-3 rounded-lg font-bold bg-orange-600 hover:bg-orange-500 flex items-center gap-2"
              >
                ↩️ 전체 되돌리기
              </button>
              <button
        ...
        {generateLog.length > 0 && (
          <div className="bg-slate-800 rounded-lg p-4 mb-6 border border-slate-700 max-h-60 overflow-y-auto">
            <div className="flex justify-between items-center mb-2">
              <h3 className="font-bold text-sm">📋 생성 로그</h3>
              {!isGenerating && (
                <button
