  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      <div className="mx-auto max-w-5xl">
        {/* BTS-0003093: BTS 바로가기 복원 */}
        <div className="mb-6 rounded-xl border border-indigo-500/30 bg-indigo-500/10 p-4 backdrop-blur">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-semibold text-indigo-100">BTS (Bug/Spec) 바로가기</p>
              <p className="text-xs text-slate-200">MySQL bugs 테이블 기반의 버그/스펙 현황을 바로 확인합니다.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link
                href="/admin/bts"
                className="rounded-lg bg-indigo-600 px-4 py-2 text-xs sm:text-sm font-semibold text-white transition hover:bg-indigo-500"
              >
                BTS 열기
              </Link>
              <Link
                href="/diagrams/bts-architecture"
                className="rounded-lg bg-slate-800 px-4 py-2 text-xs sm:text-sm font-semibold text-white transition hover:bg-slate-700 border border-slate-600/60"
              >
                BTS 아키텍처
              </Link>
            </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Link
            href="/admin/bts"
            className="group rounded-2xl border border-white/10 bg-gradient-to-br from-purple-500/20 to-slate-700/30 p-8 backdrop-blur transition hover:border-purple-500/50 hover:shadow-2xl hover:shadow-purple-500/20"
          >
            <div className="mb-4 text-5xl">🐞</div>
            <h2 className="mb-2 text-2xl font-bold text-white">버그 트래킹 (BTS)</h2>
            <p className="text-sm text-slate-300">
              버그/SPEC 현황 확인, 상태 변경, 증빙 링크 관리
            </p>
            <div className="mt-4 flex items-center text-sm font-semibold text-purple-300 transition group-hover:translate-x-2">
              바로가기 →
            </div>
          ...
          {/* 채널별 영상 통계 */}
          <Link
            href="/admin/channel-stats"
            className="group rounded-2xl border border-white/10 bg-gradient-to-br from-teal-500/20 to-cyan-500/20 p-8 backdrop-blur transition hover:border-teal-500/50 hover:shadow-2xl hover:shadow-teal-500/20"
          >
            <div className="mb-4 text-5xl">📺</div>
            <h2 className="mb-2 text-2xl font-bold text-white">채널별 영상 통계</h2>
            <p className="text-sm text-slate-300">
              YouTube 채널별 영상 분류 및 통계 현황 조회
            </p>
            <div className="mt-4 flex items-center text-sm font-semibold text-teal-300 transition group-hover:translate-x-2">
              확인하기 →
            </div>
          </Link>
          ...
          <Link
            href="/admin/bts"
            className="group rounded-2xl border border-white/10 bg-gradient-to-br from-red-500/20 to-orange-500/20 p-8 backdrop-blur transition hover:border-red-500/50 hover:shadow-2xl hover:shadow-red-500/20"
          >
            <div className="mb-4 text-5xl">🐛</div>
            <h2 className="mb-2 text-2xl font-bold text-white">버그 추적 시스템</h2>
            <p className="text-sm text-slate-300">
              버그 및 SPEC 등록, 상태 관리, 작업 할당 및 추적
            </p>
            <div className="mt-4 flex items-center text-sm font-semibold text-red-300 transition group-hover:translate-x-2">
              관리하기 →
            </div>
          ...
          >
            <div className="mb-4 text-5xl">🗺️</div>
            <h2 className="mb-2 text-2xl font-bold text-white">BTS 아키텍처</h2>
            <p className="text-sm text-slate-300">
              Bug Tracking System 구조/흐름도 (bts-architecture)
            </p>
            <div className="mt-4 flex items-center text-sm font-semibold text-indigo-300 transition group-hover:translate-x-2">
              다이어그램 보기 →
            </div>
        ...
        <div className="mt-8 rounded-xl border border-blue-500/30 bg-blue-500/10 p-6 backdrop-blur">
          ...
          <div className="space-y-2 text-sm text-slate-300">
            ...
            <p>• <strong>채널별 영상 통계:</strong> YouTube 채널별 영상 분류, 상태별 통계 및 목록 조회</p>
            <p>• <strong>버그 추적 시스템:</strong> 버그/SPEC 등록, 상태 관리, AI 에이전트 작업 할당</p>
