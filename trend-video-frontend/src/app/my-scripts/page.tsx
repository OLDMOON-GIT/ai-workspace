...
export default function MyScriptsPage() {
  ...
  const [editingScenes, setEditingScenes] = useState<Array<{ narration: string; image_prompt: string }>>([]);
  ...
  const handleEditScript = async (scriptId: string) => {
    try {
      const response = await fetch(`/api/scripts/${scriptId}/story`, {
        credentials: 'include'
      });
      if (response.ok) {
        const data = await response.json();
        if (data.scenes && data.scenes.length > 0) {
          setEditingScenes(data.scenes.map((s: any) => ({
          ...
          alert('수정 가능한 씬이 없습니다.');
        }
      } else {
        alert('대본을 불러올 수 없습니다. (story.json 없음)');
      }
    } catch (error) {
      console.error('대본 로드 오류:', error);
      alert('대본 로드 중 오류가 발생했습니다.');
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    setEditSaving(true);
    try {
      const response = await fetch(`/api/scripts/${editingScriptId}/story`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ scenes: editingScenes })
      });
      if (response.ok) {
        alert('대본이 저장되었습니다.');
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="ml-4 flex flex-col gap-2">
                    ...
                    <button
                      ...
                      className="rounded-lg bg-yellow-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-yellow-500"
                    >
                      ✏️ 수정
                    </button>
                    <button
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-4 border-b border-slate-700 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white">대본 수정</h2>
              <button
                onClick={() => { setEditModalOpen(false); setEditingScriptId(null); setEditingScenes([]); }}
            ...
            <div className="p-4 overflow-y-auto flex-1 space-y-4">
              {editingScenes.map((scene, idx) => (
                <div key={idx} className="bg-slate-700/50 rounded-lg p-4 space-y-3">
                  <div className="text-sm font-semibold text-purple-300">씬 {idx + 1}</div>
                  <div>
                    <label className="block text-xs text-slate-400 mb-1">나레이션</label>
                    <textarea
                      ...
                      className="w-full bg-slate-900 text-white rounded p-2 text-sm min-h-[80px] resize-y"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-slate-400 mb-1">이미지 프롬프트</label>
                    <textarea
                      ...
                      className="w-full bg-slate-900 text-white rounded p-2 text-sm min-h-[60px] resize-y"
                    />
                  </div>
                </div>
              ))}
