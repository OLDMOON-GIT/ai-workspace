...
interface Job {
  id: string;
  status: 'draft' | 'script' | 'video' | 'completed' | 'failed';  // BTS-3363: 3단계로 단순화
  progress: number;
  step: string;
  videoPath?: string;
...
export default function MyVideosPage() {
  ...
  const handleCancelJob = async (taskId: string) => {
    ...
    console.log('🛑 취소 요청 시작:', taskId);
    const url = `/api/generate-video-upload?taskId=${taskId}`;
  ...
  const handleOpenFolder = async (taskId: string) => {
    ...
    try {
      const response = await fetch(`/api/open-folder?taskId=${taskId}`, {
        method: 'POST',
        headers: getAuthHeaders(),
        credentials: 'include'
      });
