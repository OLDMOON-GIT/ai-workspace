...
describe('Shorts Conversion - Regression Tests', () => {
  ...
  describe('State Management Tests', () => {
    test('should add taskId to convertingJobs set when conversion starts', () => {
      const convertingJobs = new Set<string>();
      convertingJobs.add(mockJobId);
    ...
    test('should remove taskId from convertingJobs set when conversion completes', () => {
      const convertingJobs = new Set<string>([mockJobId]);
      convertingJobs.delete(mockJobId);
    ...
    test('should remove taskId from convertingJobs set when conversion fails', () => {
      const convertingJobs = new Set<string>([mockJobId]);
    ...
    test('should handle multiple conversions independently', () => {
      ...
      convertingJobs.add(taskId1);
      convertingJobs.add(taskId2);

      expect(convertingJobs.has(taskId1)).toBe(true);
      expect(convertingJobs.has(taskId2)).toBe(true);
      expect(convertingJobs.size).toBe(2);

      // Complete one job
      convertingJobs.delete(taskId1);

      expect(convertingJobs.has(taskId1)).toBe(false);
      expect(convertingJobs.has(taskId2)).toBe(true);
      expect(convertingJobs.size).toBe(1);
    });
  });
  ...
  describe('API Call Tests', () => {
    ...
    test('should handle successful API response', async () => {
      const mockResponse = { taskId: 'new-job-id' };
      (global.fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
      ...
      expect(data.taskId).toBe('new-job-id');
    });
