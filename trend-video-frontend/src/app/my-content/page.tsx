...
interface Script {
  ...
  content: string;
  status: 'draft' | 'script' | 'video' | 'completed' | 'failed' | 'pending' | 'processing';  // BTS-3408: pending/processing 추가
  progress: number;
  error?: string;
  type?: 'longform' | 'shortform' | 'sora2' | 'product' | 'product-info';  // 콘텐츠 타입
...
interface Job {
  id: string;
  status: 'pending' | 'processing' | 'completed' | 'failed' | 'cancelled' | 'video' | 'script' | 'draft';  // BTS-3408: video/script/draft 추가
  progress: number;
  step: string;
  videoPath?: string;
  ...
  youtubeUrl?: string | null;  // 유튜브 업로드 URL (단일, 호환성)
...
type TabType = 'all' | 'videos' | 'scripts' | 'published';
...
export default function MyContentPage() {
  ...
  useEffect(() => {
    ...
    if (tab && ['all', 'videos', 'scripts', 'published'].includes(tab)) {
      setActiveTab(tab);
    }
  ...
  // ⭐ 영상 대본 보기 모달 상태
  const [videoScriptModal, setVideoScriptModal] = useState<{
    videoId: string;
    title: string;
    content: string;
  } | null>(null);
    ...
    processingScripts.forEach(script => {
      const interval = setInterval(async () => {
        try {
          ...
          // 404 에러 처리 - 스크립트가 삭제되었거나 존재하지 않음
          if (response.status === 404) {
            clearInterval(interval);
            // 목록에서 해당 스크립트 제거
            setScripts(prev => prev.filter(s => s.id !== script.id));
            console.log(`Script ${script.id} not found, removed from list`);
            return;
          }
  ...
  useEffect(() => {
    ...
    const intervals: NodeJS.Timeout[] = [];
    const logOffsets: Record<string, number> = {}; // 각 job별 로그 offset 추적

    processingJobs.forEach(job => {
      logOffsets[job.id] = job.logs?.length || 0; // 현재 로그 개수로 초기화

      const interval = setInterval(async () => {
        try {
          // logOffset으로 새 로그만 요청 (대역폭 절약)
          const currentOffset = logOffsets[job.id] || 0;
          const response = await fetch(`/api/generate-video-upload?taskId=${job.id}&logOffset=${currentOffset}`, {
            headers: getAuthHeaders()
          });
          const statusData = await safeJsonResponse(response);
          ...
          if (statusData.status === 'completed' || statusData.status === 'failed') {
            ...
            // 진행 중인 작업만 업데이트 - 로그는 append
            setJobs(prev => prev.map(j => {
              if (j.id !== job.id) return j;

              // 새 로그 append
              const newLogs = statusData.logs && Array.isArray(statusData.logs) && statusData.logs.length > 0
                ? [...(j.logs || []), ...statusData.logs]
                : j.logs;

              // offset 업데이트
              if (statusData.logOffset) {
                logOffsets[job.id] = statusData.logOffset;
              } else if (statusData.logs?.length) {
                logOffsets[job.id] = (logOffsets[job.id] || 0) + statusData.logs.length;
              }
  ...
  const handleCancelScript = async (scriptId: string, title: string) => {
    ...
    showConfirmModal(
      '대본 생성 중지',
      `"${title}" 대본 생성을 중지하시겠습니까?`,
      async () => {
        console.log('✅ 중지 확인됨, API 호출 시작');
        try {
          // ⚠️ /api/scripts/[id] DELETE = 중지 (상태만 cancelled로 변경, 데이터 삭제 안 함)
          // /api/my-scripts DELETE = 실제 삭제 (데이터 삭제됨)
  ...
  const handleDeleteAllContent = async () => {
    const totalCount = total + scriptsTotal;
    if (totalCount === 0) {
      toast.error('삭제할 콘텐츠가 없습니다.');
      return;
    }

    showConfirmModal(
      '전체 삭제',
      `모든 콘텐츠 (영상 ${total}개, 대본 ${scriptsTotal}개, 총 ${totalCount}개)를 삭제하시겠습니까?\n\n📋 자동화 스케줄도 함께 삭제됩니다.\n\n⚠️ 이 작업은 되돌릴 수 없습니다!`,
      async () => {
        const loadingToast = toast.loading('전체 콘텐츠 삭제 중...');
        try {
          const response = await fetch('/api/my-content/delete-all', {
            method: 'DELETE',
            headers: getAuthHeaders(),
            credentials: 'include'
          });
          ...
          if (response.ok) {
            const scheduleMsg = data.deletedSchedules > 0 ? `, 스케줄: ${data.deletedSchedules}개` : '';
            toast.success(`✅ 전체 삭제 완료 (영상: ${data.deletedJobs}개, 대본: ${data.deletedScripts}개${scheduleMsg})`, {
              id: loadingToast
            });
            // 목록 초기화
            setJobs([]);
            ...
            toast.error('전체 삭제 실패: ' + (data.error || '알 수 없는 오류'), {
              id: loadingToast
            });
          }
        } catch (error) {
  ...
  const toggleContent = (scriptId: string) => {
    if (expandedScriptId === scriptId) {
      ...
      setExpandedScriptLogId(null); // 대본 열면 로그 닫기
    }
  };

  const toggleLog = (scriptId: string) => {
    if (expandedScriptLogId === scriptId) {
      setExpandedScriptLogId(null);
    } else {
      setExpandedScriptLogId(scriptId);
      setExpandedScriptId(null); // 로그 열면 대본 닫기
    }
  };
  ...
  const handleCancelJob = async (taskId: string) => {
    showConfirmModal(
      ...
      async () => {
        try {
          const response = await fetch(`/api/generate-video-upload?taskId=${taskId}`, {
            method: 'DELETE',
            headers: getAuthHeaders(),
            credentials: 'include'
          });
  ...
  const handleDeleteVideo = async (taskId: string, title: string) => {
    showConfirmModal(
      '영상 삭제',
      `"${title}" 영상을 삭제하시겠습니까?`,
      async () => {
        try {
          const response = await fetch(`/api/my-videos?taskId=${taskId}`, {
            method: 'DELETE',
            headers: getAuthHeaders(),
            credentials: 'include'
          });
  ...
  const handleRestartVideo = async (taskId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 새로운 taskId의 로그를 자동으로 열기
        if (data.taskId) {
          setExpandedLogJobId(data.taskId);
        }
      } else {
  ...
  const handleConvertToShorts = async (taskId: string, title: string) => {
    // 이미 변환 중이면 무시
    if (convertingJobs.has(taskId)) {
    ...
    showConfirmModal(
      ...
      async () => {
        // 즉시 변환 중 상태로 설정
        setConvertingJobs(prev => new Set(prev).add(taskId));

        // 즉시 토스트 표시
        const toastId = toast.loading('🎬 쇼츠 변환 시작 중...');

        try {
          const response = await fetch(`/api/jobs/${taskId}/convert-to-shorts`, {
            method: 'POST',
            headers: {
              ...getAuthHeaders(),
              'Content-Type': 'application/json'
  ...
  const handleConvertToShortform = async (taskId: string, title: string) => {
    // 이미 변환 중이면 무시
    if (convertingJobs.has(taskId)) {
      toast.error('이미 변환 중입니다.');
      return;
    ...
    showConfirmModal(
      '⚡ 숏폼으로 변환',
      `"${title}"\n\n━━━━━━━━━━━━━━━━━━━━━━\n💰 크레딧 차감: 200 크레딧\n━━━━━━━━━━━━━━━━━━━━━━\n\n📝 대본을 요약하여 2분 30초 분량으로 압축\n🎬 씬 개수 유지 (원본과 동일)\n🖼️ 원본 이미지를 9:16으로 자동 변환\n✨ 마지막에 CTA 자동 추가 (롱폼 링크 안내, 구독 요청)\n\n영상을 2분 30초 숏폼으로 변환하시겠습니까?`,
      async () => {
        // 즉시 변환 중 상태로 설정
        setConvertingJobs(prev => new Set(prev).add(taskId));
        ...
        try {
          ...
          const response = await fetch(`/api/jobs/${taskId}/convert-to-shortform`, {
            method: 'POST',
            headers: {
              ...getAuthHeaders(),
              'Content-Type': 'application/json'
  ...
  const handleRestartScript = async (scriptId: string, title: string) => {
    ...
    if (script.promptFormat) {
      params.set('retryType', script.promptFormat);
    }

    // 상품 대본인 경우 DB에 저장된 productInfo를 localStorage에 저장
    if ((script.promptFormat === 'product' || script.promptFormat === 'product-info') && (script as any).productInfo) {
      try {
  ...
  const handleImageCrawling = async (scriptId: string, taskId?: string, useImageFX: boolean = false) => {
    ...
    try {
      ...
      // format 정보 추출 (metadata 또는 script.promptFormat 사용)
      const format = scriptData.metadata?.format || script.promptFormat || 'shortform';
      console.log(`📐 [ImageCrawl] 대본 포맷: ${format}, script.promptFormat: ${script.promptFormat}`);

      // API 호출 (credentials 추가)
      // ⭐ product_info와 metadata도 전달 (상품 썸네일 업로드용)
      const productInfo = scriptData.product_info;
      const metadata = scriptData.metadata;
      const category = metadata?.category;

      console.log(`🛒 [ImageCrawl] product_info:`, productInfo ? 'yes' : 'no');
      console.log(`📂 [ImageCrawl] category:`, category);
      ...
      const response = await fetch('/api/images/crawl', {
        ...
        body: JSON.stringify({
          ...
          useImageFX,
          format,  // ⭐ 포맷 정보 전달
          productInfo,  // ⭐ 상품 정보 (썸네일 포함)
          metadata,     // ⭐ 메타데이터 (카테고리 포함)
          category      // ⭐ 카테고리 직접 전달
        })
      });
  ...
  // ⭐ 영상 대본 보기
  const handleViewVideoScript = async (videoId: string, title: string) => {
    try {
      const res = await fetch(`/api/automation/get-story?scriptId=${videoId}`);
      const data = await res.json();

      if (data.success && data.storyJson) {
        setVideoScriptModal({
          videoId,
          title,
          content: JSON.stringify(data.storyJson, null, 2)
        });
      } else {
        toast.error('대본을 불러올 수 없습니다: ' + (data.error || '알 수 없는 오류'));
      }
    } catch (error: any) {
      console.error('❌ 대본 로드 오류:', error);
      toast.error('대본 로드 중 오류가 발생했습니다.');
    }
  };

  const handleOpenFolder = async (taskId: string, videoPath?: string) => {
    console.log('📁 폴더 열기 버튼 클릭됨, taskId:', taskId);

    try {
      const params = new URLSearchParams();
      params.set('taskId', taskId);
      if (videoPath) {
        params.set('path', videoPath);
      }

      const response = await fetch(`/api/open-folder?${params.toString()}`, {
        method: 'POST',
        headers: getAuthHeaders(),
        credentials: 'include'
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      <div className="mx-auto max-w-6xl">
        {/* 탭 */}
        <div className="mb-6 flex flex-wrap gap-2 sm:gap-3 justify-between items-center">
          <div className="flex flex-wrap gap-2 sm:gap-3">
            <button
              onClick={() => handleTabChange('all')}
              className={`rounded-lg px-3 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm font-semibold transition-all duration-200 ease-in-out whitespace-nowrap ${
                activeTab === 'all'
                  ? 'bg-purple-600 text-white shadow-lg shadow-purple-500/30 scale-105'
                  : 'bg-white/10 text-slate-300 hover:bg-white/20 hover:scale-[1.02]'
              }`}
            >
          ...
          {(total + scriptsTotal) > 0 && (
            <button
              ...
              className="rounded-lg px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold bg-red-600 text-white transition hover:bg-red-700 whitespace-nowrap"
              title="모든 영상과 대본을 삭제합니다"
            >
              🗑️ 전체 삭제
            </button>
          )}
        ...
        {/* 전체 탭 콘텐츠 */}
        {activeTab === 'all' && (
          <div key="all-tab" className="animate-fadeIn">
            {/* 검색 */}
            ...
            <div className="mb-6 flex flex-wrap gap-2 sm:gap-3">
              <button
                ...
                className={`rounded-lg px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold transition-all duration-200 whitespace-nowrap ${
                  filter === 'all'
            ...
            {(isLoadingScripts || isLoadingVideos) ? (
              ...
              <div className="space-y-4 stagger-children">
                ...
                {(() => {
                  ...
                  // ⭐ 중복 제거: 같은 ID가 jobs와 scripts 양쪽에 있으면 video 우선 (jobs만 표시)
                  const jobIds = new Set(filteredJobs.map(job => job.id));
                  const dedupedScripts = filteredScripts.filter(script => !jobIds.has(script.id));

                  const allItems = [
                    ...filteredJobs.map(job => ({ type: 'video' as const, data: job, date: job.createdAt })),
                    ...dedupedScripts.map(script => ({ type: 'script' as const, data: script, date: script.createdAt }))
                  ...
                  return (
                    ...
                    <div
                      ...
                      className={`group rounded-xl border transition-all duration-200 overflow-hidden animate-card ${
                    ...
                    >
                        ...
                        <div className="flex flex-col md:flex-row gap-4 p-4">
                          ...
                          <div className="relative w-full md:w-64 h-36 flex-shrink-0 bg-slate-800/50 rounded-lg overflow-hidden">
                            {item.data.status === 'completed' && item.data.thumbnailPath ? (
                              <a
                                href={`/api/download-thumbnail?taskId=${item.data.id}`}
                                download
                                className="block w-full h-full cursor-pointer group/thumb relative"
                                title="클릭하여 썸네일 다운로드"
                              >
                                <img
                                  src={`/api/thumbnail?taskId=${item.data.id}`}
                                  alt="썸네일"
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-between">
                            <div>
                              <div className="flex items-start gap-2 mb-2">
                                ...
                                {/* 타입 배지 - promptFormat 또는 type 사용 */}
                                {((item.data as any).promptFormat || (item.data as any).type) && (
                                  <span className={`px-2 py-1 rounded text-xs font-bold shadow-lg flex-shrink-0 ${
                                    ((item.data as any).promptFormat || (item.data as any).type) === 'shortform' ? 'bg-blue-500 text-white' :
                                    ((item.data as any).promptFormat || (item.data as any).type) === 'longform' ? 'bg-green-500 text-white' :
                                    ((item.data as any).promptFormat || (item.data as any).type) === 'product' ? 'bg-orange-500 text-white' :
                                    ((item.data as any).promptFormat || (item.data as any).type) === 'sora2' ? 'bg-purple-500 text-white' :
                                    'bg-gray-500 text-white'
                                  }`}>
                                    {((item.data as any).promptFormat || (item.data as any).type) === 'shortform' ? '⚡ 숏폼' :
                                     ((item.data as any).promptFormat || (item.data as any).type) === 'longform' ? '📝 롱폼' :
                                     ((item.data as any).promptFormat || (item.data as any).type) === 'product' ? '🛍️ 상품' :
                                     ((item.data as any).promptFormat || (item.data as any).type) === 'product-info' ? '📝 상품설명' :
                                     ((item.data as any).promptFormat || (item.data as any).type) === 'sora2' ? '🎬 Sora2' :
                                     ((item.data as any).promptFormat || (item.data as any).type)}
                                  </span>
                                )}
                                {/* 롱폼→숏폼 변환 배지 */}
                                {(item.data as any).promptFormat === 'shortform' && item.data.sourceContentId && (
                                  <span className="px-2 py-1 rounded text-xs font-bold shadow-lg flex-shrink-0 bg-cyan-500 text-white">
                                    🔄 롱폼변환
                                  </span>
                                )}
                                {/* 상태 배지 */}
                              ...
                              <div className="space-y-1 text-sm text-slate-400">
                                ...
                                {/* YouTube 링크 표시 */}
                                {item.data.youtubeUrls && item.data.youtubeUrls.length > 0 ? (
                                  item.data.youtubeUrls.map((url, idx) => (
                                    <p key={`yt-info-${item.data.id}-${idx}`} className="flex items-center gap-2">
                                      <span className="text-slate-500">•</span>
                                      <a
                                        href={url}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="text-red-400 hover:text-red-300 underline transition text-xs truncate max-w-[300px]"
                                      >
                                        {url}
                                      </a>
                                    </p>
                                ...
                                {/* From 링크 (대본에서 생성된 영상인 경우) - 숏폼은 별도 링크 사용 */}
                                {item.data.sourceContentId && (item.data as any).promptFormat !== 'shortform' && (
                                  <p className="flex items-center gap-2">
                                    <span className="text-slate-500">•</span>
                                    <span>
                                      From:{' '}
                                      <button
                                        onClick={() => {
                                          const scriptId = item.data.sourceContentId;
                                          // 먼저 현재 화면(DOM)에서 찾기
                                          const sourceElement = document.getElementById(`script-${scriptId}`);
                                          if (sourceElement) {
                                            // 현재 화면에 있으면 바로 스크롤
                                ...
                                {/* ⭐ promptFormat이 shortform이고 sourceContentId가 있으면 롱폼→숏폼 변환 */}
                                {((item.data as any).promptFormat === 'shortform' && item.data.sourceContentId) && (
                                  <p className="flex items-center gap-2">
                                    <span className="text-slate-500">•</span>
                                    <span>
                                      ...
                                      <button
                                        onClick={async () => {
                                          // ⭐ 롱폼 YouTube URL 조회 시도
                                          try {
                                            const res = await fetch(`/api/jobs/${item.data.id}/longform-url`, {
                                              headers: getAuthHeaders()
                                            });
                                            if (res.ok) {
                                              const data = await res.json();
                                              if (data.success && data.longformUrl) {
                                                // YouTube 링크 열기
                                                window.open(data.longformUrl, '_blank');
                                                return;
                                              }
                                            }
                                          } catch (err) {
                                            console.warn('롱폼 URL 조회 실패:', err);
                                          }

                                          // fallback: 페이지 내 스크롤
                                          const sourceElement = document.getElementById(`video-${item.data.sourceContentId}`);
                                          if (sourceElement) {
                                            sourceElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                            sourceElement.classList.add('ring-2', 'ring-purple-500', 'ring-offset-2', 'ring-offset-slate-900');
                                            setTimeout(() => {
                                            ...
                                            toast.error('원본 롱폼 영상을 찾을 수 없습니다.');
                                          }
                                        }}
                                        className="text-purple-400 hover:text-purple-300 underline cursor-pointer transition"
                                      >
                            ...
                            {(item.data.status === 'pending' || item.data.status === 'processing') && (
                              <>
                                <button
                                  onClick={() => handleOpenFolder(item.data.id, item.data.videoPath)}
                            ...
                            {item.data.status === 'completed' && (
                              <>
                                ...
                                <button
                                  onClick={() => handleViewVideoScript(item.data.id, item.data.title || '영상 대본')}
                                  className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                                  title="대본 보기"
                                >
                                  📄 대본
                                ...
                                {item.data.videoPath && (
                                  <>
                                    ...
                                    {item.data.sourceContentId && (() => {
                                      ...
                                      return sourceScript && sourceScript.content ? (
                                        <button
                                          ...
                                          className={`px-3 py-1.5 text-white rounded text-sm font-medium whitespace-nowrap ${
                                    ...
                                    {/* === 편집 === */}
                                    <a
                                      href={`/api/download-video?taskId=${item.data.id}`}
                                      download
                                    ...
                                    {/* 숏폼 버튼: 롱폼 영상에만 표시 */}
                                    {((item.data as any).promptFormat === 'longform' || item.data.type === 'longform') && (
                                      <button
                                ...
                                {/* === 관리 버튼 (비디오 유무와 관계없이 표시) === */}
                                <button
                                  onClick={() => handleRestartVideo(item.data.id)}
                                  className="px-3 py-1.5 bg-orange-600 hover:bg-orange-500 text-white rounded text-sm font-medium whitespace-nowrap"
                                  title="영상 재생성"
                                >
                                  🔄 재시도
                                </button>
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-between">
                            <div>
                              ...
                              <div className="space-y-1 text-sm text-slate-400">
                                ...
                                {item.data.status === 'completed' && item.data.content && (
                                  <p className="flex items-center gap-2">
                                    <span className="text-slate-500">•</span>
                              ...
                              {(item.data.status === 'pending' || item.data.status === 'processing') && (
                                <>
                                  <button
                                    onClick={() => toggleLog(item.data.id)}
                                    className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                                    title="로그 보기"
                                  >
                                  ...
                                  {item.data.type !== 'product-info' && (
                                    <button
                                      onClick={async () => {
                                        ...
                                        console.log('📝 대본 ID:', item.data.id);
                                        ...
                                        try {
                                          ...
                                          // 폴더에 있는 미디어 파일 확인
                                          let existingMedia: Array<{ name: string; url: string; type: 'image' | 'video' }> = [];
                                          try {
                                            const mediaRes = await fetch(`/api/tasks/${item.data.id}/media`);
                                            if (mediaRes.ok) {
                                              const mediaData = await mediaRes.json();
                                              if (mediaData.success && mediaData.media && mediaData.media.length > 0) {
                                                existingMedia = mediaData.media;
                                                console.log(`📁 폴더에서 미디어 ${existingMedia.length}개 발견:`, existingMedia.map((m: any) => m.name));
                                              }
                                            }
                                          } catch (mediaErr) {
                                            console.log('📁 미디어 확인 실패 (무시하고 진행):', mediaErr);
                                          }

                                          // 로컬 스토리지에 저장 (포맷 타입 + 미디어 정보 포함)
                                          const pipelineData = {
                                            title: item.data.title,
                                            content: scriptJson,
                                            type: item.data.type || 'longform', // 기본값은 longform
                                            scriptId: item.data.id, // 대본 ID (폴더 경로용)
                                            existingMedia: existingMedia // 폴더에 있는 미디어 파일들
                                          };
                                  ...
                                  <button
                                    onClick={async () => {
                                      try {
                                        ...
                                        const folderPath = item.data.id.replace(/^(task_|title_|script_)/, '');
                                    ...
                                    className="flex items-center justify-center gap-1 px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                                    title="대본 폴더 열기"
                                  >
                                    📁 폴더
                                  </button>
                                  {/* 변환 버튼: longform/shortform 타입에만 표시 */}
                                  {((item.data as any).promptFormat === 'longform' || (item.data as any).promptFormat === 'shortform' || item.data.type === 'longform' || item.data.type === 'shortform') && (
                                    <button
                                      onClick={() => handleConvertScript(item.data.id, (item.data as any).promptFormat || item.data.type || 'longform', item.data.title)}
                                      className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                                      title={((item.data as any).promptFormat || item.data.type) === 'longform' ? '쇼츠로 변환' : '롱폼으로 변환'}
                                    >
                                      🔄 변환
                                    </button>
                                  )}
                      ...
                      {/* 영상 로그 */}
                      {item.type === 'video' && expandedLogJobId === item.data.id && (
                        <div className="mt-4 rounded-lg border border-slate-600 bg-slate-900/80 p-4">
                          <div className="mb-3 flex items-center justify-between">
                            <span className="text-sm font-bold text-slate-300">📋 서버 로그</span>
                            <span className="text-sm text-slate-400">{item.data.logs?.length || 0}개 항목</span>
                          </div>
                          {(!item.data.logs || item.data.logs.length === 0) ? (
                            <p className="text-sm text-slate-400">로그가 없습니다.</p>
                          ) : (
        ...
        {/* 대본 탭 콘텐츠 */}
        {activeTab === 'scripts' && (
          <div key="scripts-tab" className="animate-fadeIn">
            {/* 검색 */}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                          <div className="flex items-start gap-2 mb-2">
                            ...
                            {script.promptFormat && (
                              <span className={`px-2 py-1 rounded text-xs font-bold shadow-lg flex-shrink-0 ${
                                script.promptFormat === 'shortform' ? 'bg-blue-500 text-white' :
                                script.promptFormat === 'longform' ? 'bg-green-500 text-white' :
                                script.promptFormat === 'product' ? 'bg-orange-500 text-white' :
                                'bg-purple-500 text-white'
                              }`}>
                                {script.promptFormat === 'shortform' ? '⚡ 숏폼' :
                                 script.promptFormat === 'longform' ? '📝 롱폼' :
                                 script.promptFormat === 'product' ? '🛍️ 상품' :
                                 script.promptFormat === 'product-info' ? '📝 상품설명' :
                                 '🎬 Sora2'}
                              </span>
                            )}
                            {/* 상태 배지 */}
                          ...
                          <div className="space-y-1 text-sm text-slate-400">
                            ...
                            {/* 카테고리 표시 */}
                            {((script as any).category || script.promptFormat === 'product-info') && (
                              <p className="flex items-center gap-2">
                                <span className="text-slate-500">•</span>
                                <span className="inline-flex items-center gap-1">
                                  <span className="text-purple-400">🎭</span>
                                  <span className="text-purple-300 font-medium">
                                    {script.promptFormat === 'product-info' ? '상품설명' : (script as any).category}
                                  </span>
                                </span>
                              </p>
                            )}
                        ...
                        {script.status === 'completed' && (
                          <>
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  let existingMedia: Array<{ name: string; url: string; type: 'image' | 'video' }> = [];
                                  try {
                                    const mediaRes = await fetch(`/api/tasks/${script.id}/media`);
                                    if (mediaRes.ok) {
                                      const mediaData = await mediaRes.json();
                                      if (mediaData.success && mediaData.media && mediaData.media.length > 0) {
                                        existingMedia = mediaData.media;
                                  ...
                                  const pipelineData = {
                                    title: script.title,
                                    content: scriptJson,
                                    type: script.promptFormat || 'longform', // 기본값은 longform
                                    scriptId: script.id, // 대본 ID (폴더 경로용)
                                    existingMedia: existingMedia // 폴더에 있는 미디어 파일들
                                  };
                                  localStorage.setItem('pipelineScript', JSON.stringify(pipelineData));
                                  console.log('💾 localStorage에 저장 완료');
                            ...
                            {/* ⚠️ DEPRECATED: 상품정보 별도 생성 제거됨 (대본에 youtube_description 자동 포함) */}
                            ...
                            <button
                              onClick={async () => {
                                try {
                                  // ⚠️ ID 규칙: prefix 없이 순수 ID만 사용
                                  const folderPath = script.id.replace(/^(task_|title_|script_)/, '');
                                  const response = await fetch(`/api/open-folder?path=${encodeURIComponent(folderPath)}`, {
                                    headers: getAuthHeaders(),
                                    credentials: 'include'
                                  });
                            ...
                            {/* 변환 버튼: longform/shortform 타입에만 표시 */}
                            {(script.promptFormat === 'longform' || script.promptFormat === 'shortform') && (
                              <button
                                onClick={() => handleConvertScript(script.id, script.promptFormat || 'longform', script.title)}
                                className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                                title={script.promptFormat === 'longform' ? '쇼츠로 변환' : '롱폼으로 변환'}
                              >
                                🔄 변환
                              </button>
                            )}
                        ...
                        {(script.status === 'failed' || (script.status as any) === 'cancelled') && (
                          <>
                            ...
                            <button
                              onClick={() => handleOpenFolder(script.id)}
                              className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                              title="폴더 열기"
                            >
                              📁 폴더
        ...
        {/* 영상 탭 콘텐츠 */}
        {activeTab === 'videos' && (
          <div key="videos-tab" className="animate-fadeIn">
            {/* 검색 */}
            <div className="mb-4 flex flex-col sm:flex-row gap-2">
              <input
                type="text"
                    ...
                    <div className="flex flex-col md:flex-row gap-4 p-4">
                      ...
                      <div className="relative w-full md:w-64 h-36 flex-shrink-0 bg-slate-800/50 rounded-lg overflow-hidden">
                        {job.status === 'completed' && job.thumbnailPath ? (
                          <a
                            href={`/api/download-thumbnail?taskId=${job.id}`}
                            download
                            className="block w-full h-full cursor-pointer group/thumb relative"
                            title="클릭하여 썸네일 다운로드"
                          >
                            <img
                              src={`/api/thumbnail?taskId=${job.id}`}
                              alt="썸네일"
                              className="w-full h-full object-cover"
                            />
                            {/* 다운로드 아이콘 오버레이 */}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                          <div className="flex items-start gap-2 mb-2">
                            ...
                            {/* 타입 배지 - promptFormat 또는 type 사용 */}
                            {((job as any).promptFormat || job.type) && (
                              <span className={`px-2 py-1 rounded text-xs font-bold shadow-lg flex-shrink-0 ${
                                ((job as any).promptFormat || job.type) === 'shortform' ? 'bg-blue-500 text-white' :
                                ((job as any).promptFormat || job.type) === 'longform' ? 'bg-green-500 text-white' :
                                ((job as any).promptFormat || job.type) === 'product' ? 'bg-orange-500 text-white' :
                                ((job as any).promptFormat || job.type) === 'sora2' ? 'bg-purple-500 text-white' :
                                'bg-gray-500 text-white'
                              }`}>
                                {((job as any).promptFormat || job.type) === 'shortform' ? '⚡ 숏폼' :
                                 ((job as any).promptFormat || job.type) === 'longform' ? '📝 롱폼' :
                                 ((job as any).promptFormat || job.type) === 'product' ? '🛍️ 상품' :
                                 ((job as any).promptFormat || job.type) === 'product-info' ? '📝 상품설명' :
                                 ((job as any).promptFormat || job.type) === 'sora2' ? '🎬 Sora2' :
                                 ((job as any).promptFormat || job.type)}
                              </span>
                            )}
                            {/* 롱폼→숏폼 변환 배지 */}
                            {(job as any).promptFormat === 'shortform' && job.sourceContentId && (
                              <span className="px-2 py-1 rounded text-xs font-bold shadow-lg flex-shrink-0 bg-cyan-500 text-white">
                                🔄 롱폼변환
                              </span>
                            )}
                            ...
                            {/* 자동화 큐 배지 */}
                            {(job as Job).automationQueue?.inQueue && (
                              <span className={`px-2 py-1 rounded text-xs font-bold shadow-lg flex-shrink-0 ${
                                (job as Job).automationQueue?.queueStatus === 'pending' ? 'bg-yellow-500 text-black' :
                                (job as Job).automationQueue?.queueStatus === 'processing' ? 'bg-blue-500 text-white' :
                                (job as Job).automationQueue?.queueStatus === 'waiting_for_upload' ? 'bg-purple-500 text-white' :
                                (job as Job).automationQueue?.queueStatus === 'cancelled' ? 'bg-gray-500 text-white' :
                                'bg-green-500 text-white'
                              }`}>
                                {(job as Job).automationQueue?.queueStatus === 'pending' ? '⏳ 큐 대기' :
                                 (job as Job).automationQueue?.queueStatus === 'processing' ? '⚙️ 자동화 중' :
                                 (job as Job).automationQueue?.queueStatus === 'waiting_for_upload' ? '📤 업로드 대기' :
                                 (job as Job).automationQueue?.queueStatus === 'cancelled' ? '❌ 큐 취소됨' :
                                 '✅ 자동화 완료'}
                              </span>
                            )}
                          </div>
                          <div className="space-y-1 text-sm text-slate-400">
                            ...
                            {(job as any).category && (
                              <p className="flex items-center gap-2">
                                ...
                                <span className="inline-flex items-center gap-1">
                                  <span className="text-purple-400">🎭</span>
                                  <span className="text-purple-300 font-medium">{(job as any).category}</span>
                                </span>
                              </p>
                            )}
                            {/* YouTube 링크 표시 */}
                            {job.youtubeUrls && job.youtubeUrls.length > 0 ? (
                              job.youtubeUrls.map((url, idx) => (
                                <p key={`yt-info-${job.id}-${idx}`} className="flex items-center gap-2">
                                  <span className="text-slate-500">•</span>
                                  <a
                                    href={url}
                                    target="_blank"
                            ...
                            {job.sourceContentId && (job as any).promptFormat !== 'shortform' && (
                              <p className="flex items-center gap-2">
                                <span className="text-slate-500">•</span>
                                <span>
                                  From:{' '}
                            ...
                            {/* 숏폼인 경우 원본 롱폼 보기 링크 */}
                            {job.type === 'shortform' && (job as any).convertedFromJobId && (
                              <p className="flex items-center gap-2">
                                <span className="text-slate-500">•</span>
                                <span>
                                  <button
                                    onClick={async () => {
                                      // ⭐ 롱폼 YouTube URL 조회 시도
                                      try {
                                        const res = await fetch(`/api/jobs/${job.id}/longform-url`, {
                                          headers: getAuthHeaders()
                                        });
                                        if (res.ok) {
                                          const data = await res.json();
                                      ...
                                      // fallback: 페이지 내 스크롤/검색
                                      const convertedFromId = (job as any).convertedFromJobId;
                                      const sourceElement = document.getElementById(`video-${convertedFromId}`);
                                      if (sourceElement) {
                                        sourceElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                        sourceElement.classList.add('ring-2', 'ring-purple-500', 'ring-offset-2', 'ring-offset-slate-900');
                                        setTimeout(() => {
                                        ...
                                        // DOM에 없으면 전체 탭으로 전환하고 검색
                                        setActiveTab('all');
                                        setFilter('all');
                                        setSearchQuery(convertedFromId);
                                        setActiveSearchQuery(convertedFromId);
                                        setJobs([]);
                                        setOffset(0);
                                        toast.success('원본 롱폼을 검색합니다');
                                      }
                                    }}
                                    className="text-purple-400 hover:text-purple-300 underline cursor-pointer transition"
                                  >
                        ...
                        {(job.status === 'pending' || job.status === 'processing') && (
                          <>
                            <button
                              onClick={() => handleOpenFolder(job.id, job.videoPath)}
                              className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                              title="폴더 열기"
                            >
                              📁 폴더
                        ...
                        {job.status === 'completed' && job.videoPath && (
                          <>
                            ...
                            <button
                              onClick={() => handleViewVideoScript(job.id, job.title || '영상 대본')}
                              className="px-3 py-1.5 bg-slate-600 hover:bg-slate-500 text-white rounded text-sm font-medium whitespace-nowrap"
                              title="대본 보기"
                            >
                              📄 대본
                            ...
                            {/* === 편집 === */}
                            <a
                              href={`/api/download-video?taskId=${job.id}`}
                              download
                              className="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white rounded text-sm font-medium whitespace-nowrap"
                              title="영상 다운로드"
                            >
                            ...
                            {job.type === 'longform' && (
                              <button
                                onClick={() => handleConvertToShortform(job.id, job.title || '제목 없음')}
                                ...
                                title={convertingJobs.has(job.id) ? '변환 중...' : '숏폼으로 변환 (200 크레딧)'}
                              >
                                {convertingJobs.has(job.id) ? '⏳ 변환 중...' : '📱 숏폼'}
                              </button>
                            )}
                            <button
        ...
        {/* 퍼블리시 탭 콘텐츠 */}
        {activeTab === 'published' && (
          <div key="published-tab" className="animate-fadeIn space-y-4">
            {isLoadingUploads ? (
              ...
              <div className="space-y-4 stagger-children">
                {youtubeUploads.map((upload) => (
                  <div
                    key={upload.id}
                    className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm hover:bg-white/10 transition-all duration-200 animate-card"
                  >
                    <div className="flex flex-col md:flex-row gap-4 p-4">
                      {/* 썸네일 */}
                      <div className="relative w-full md:w-64 h-36 flex-shrink-0 bg-slate-800/50 rounded-lg overflow-hidden">
      ...
      {videoScriptModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-4xl max-h-[80vh] rounded-xl bg-slate-800 shadow-2xl flex flex-col">
            <div className="border-b border-slate-700 p-6 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-white">📄 영상 대본</h2>
                <p className="mt-1 text-sm text-slate-300">{videoScriptModal.title}</p>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(videoScriptModal.content);
                    toast.success('대본이 클립보드에 복사되었습니다!');
                  }}
            ...
            <div className="flex-1 overflow-auto p-6">
              <pre className="whitespace-pre-wrap break-words text-sm text-slate-100 font-mono bg-slate-900 p-4 rounded-lg">
                {videoScriptModal.content}
              </pre>
            </div>
          </div>
        </div>
