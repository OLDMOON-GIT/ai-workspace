...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // 데이터베이스에서 사용자 작업 조회
    const userJobs = await getChineseConverterJobsByUserId(user.userId);
...
/**
 * DELETE /api/chinese-converter/jobs?taskId=xxx
 * 중국영상변환 작업 삭제
 */
export async function DELETE(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // 데이터베이스에서 작업 삭제
    const deleted = await deleteChineseConverterJob(taskId);

    if (!deleted) {
      return NextResponse.json({ error: '작업 삭제 실패' }, { status: 500 });
    }

    console.log(`✅ 중국영상변환 작업 삭제: ${taskId}`);
