...
import { createJob, updateJob, flushJobLogs, findJobById, getSettings, deductCredits, addCreditHistory, addVideoLog } from '@/lib/db';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const taskId = `merge_${Date.now()}_${Math.random().toString(36).substring(7)}`;

    createJob(user.userId, taskId, jobTitle, undefined, undefined, ttsVoice);
    ...
    for (let i = 0; i < videoFiles.length; i++) {
      ...
      await addVideoLog(taskId, `  ${mediaType} [${i + 1}] ${video.name}`);
    }
    ...
    if (jsonText) {
      ...
      await addVideoLog(taskId, `📄 원본 JSON 저장: original_story.json`);
    }
    ...
    pythonProcess.stderr?.on('data', async (data) => {
      const text = data.toString('utf-8');
      stderrBuffer += text;
      await addVideoLog(taskId, `⚠️ ${text}`);
    });
