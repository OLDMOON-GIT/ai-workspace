...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    authUrl.searchParams.set('prompt', 'consent');  // 재인증 시에도 항상 consent 요청
    ...
    // fetch로 호출되는 경우(JSON 선호)에는 authUrl을 JSON으로 반환
    // 브라우저 주소창/링크 이동처럼 navigation 요청이면 기존처럼 바로 리다이렉트
    const secFetchMode = request.headers.get('sec-fetch-mode');
    const prefersJsonResponse =
      request.nextUrl.searchParams.get('response') === 'json' ||
      (secFetchMode && secFetchMode !== 'navigate') ||
      !secFetchMode; // Node fetch 등에서는 header가 비어 있음
