...
import { getChannelSettings } from '@/lib/automation';
...
/**
 * GET /api/youtube/channels - 사용자의 연결된 YouTube 채널 목록 조회 (자동화 설정 포함)
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    let channelSettings = [];
    try {
      channelSettings = await getChannelSettings(user.userId);
    } catch (error) {
      console.warn('채널 설정 로드 실패:', error);
      // 설정 로드 실패해도 채널 목록은 반환
    }
