          ...
          for (const setting of allSettings) {
            ...
            if (category === '상품') {
              try {
                ...
                // 베스트셀러 상품 중 중복되지 않은 상품 찾기
                // 1차: 상위 10개를 랜덤 순서로 확인
                // 2차: 모두 등록되어 있으면 11번째 이후 상품도 순서대로 확인
                const totalProducts = data.data.length;
                const topN = Math.min(10, totalProducts);
                let bestProduct: any = null;
                let selectedIndex = -1;

                // 상위 10개를 랜덤 순서로 시도
                const shuffledIndices = Array.from({ length: topN }, (_, i) => i)
                  .sort(() => Math.random() - 0.5);

                // 1차: 상위 10개 확인
                for (const idx of shuffledIndices) {
                  const candidate = data.data[idx];
                  sendLog(`🔍 [${idx + 1}위] 상품 확인 중: "${candidate.productName}"`);

                  // productUrl 체크
                  if (!candidate.productUrl) {
                    sendLog(`   ⚠️ productUrl 없음, 건너뜀`);
                    continue;
                  }

                  // 제목 중복 체크
                  const existingByTitle = await getOne(`
                    SELECT coupang_id, created_at, title FROM coupang_product
                    WHERE user_id = ? AND title = ?
                  `, [user.userId, candidate.productName]) as any;

                  if (existingByTitle) {
                    sendLog(`   ⏸️ 이미 등록됨 (${existingByTitle.created_at}), 다른 상품 시도...`);
                    continue;
                  }
                  ...
                  selectedIndex = idx;
                  sendLog(`✅ 중복 없는 상품 발견! (${idx + 1}위): ${candidate.productName}`);
                  break;
                }

                // 2차: 상위 10개 모두 등록되어 있으면 11번째 이후 상품 확인
                if (!bestProduct && totalProducts > topN) {
                  sendLog(`⚠️ 베스트셀러 상위 ${topN}개 상품 모두 등록됨, ${topN + 1}위 이후 상품 확인 중...`);

                  for (let idx = topN; idx < totalProducts; idx++) {
                    const candidate = data.data[idx];
                    sendLog(`🔍 [${idx + 1}위] 상품 확인 중: "${candidate.productName}"`);
                    ...
                    if (existingByTitle) {
                      sendLog(`   ⏸️ 이미 등록됨 (${existingByTitle.created_at}), 다음 상품 확인...`);
                      continue;
                    }
                ...
                // 모든 상품이 중복인 경우
                if (!bestProduct) {
                  sendLog(`❌ 베스트셀러 전체 ${totalProducts}개 상품 모두 이미 등록됨`);
                  sendLog(`   ⏭️ 다음 채널로 넘어갑니다...`);
                  skippedCount++;
                  sendLog('');
                  continue;
                }
                ...
                let deepLink: string | null = null; // 딥링크 생성 실패 시 null
                ...
                if (deeplinkResponse.ok) {
                  ...
                  if (deeplinkData.rCode === '0' && deeplinkData.data && deeplinkData.data[0]?.shortenUrl) {
                    ...
                    sendLog(`❌ 딥링크 생성안됨: ${deeplinkData.rMessage || deeplinkData.message || '알 수 없음'}`);
                    sendLog(`⚠️ 이 상품은 내 목록 / 예약큐에 등록되지 않습니다.`);
                    deeplinkFailed = true;
                  }
                } else {
                  const errorText = await deeplinkResponse.text();
                  console.error('❌ 딥링크 API HTTP 오류:', deeplinkResponse.status, errorText);
                  sendLog(`❌ 딥링크 생성안됨: HTTP ${deeplinkResponse.status}`);
                  sendLog(`⚠️ 이 상품은 내 목록 / 예약큐에 등록되지 않습니다.`);
                  deeplinkFailed = true;
                }
                ...
                // 딥링크 생성 실패 또는 형식 오류 시 다음 상품으로 넘어감
                if (deeplinkFailed || !isValidDeepLink) {
                  sendLog(`🚫 "${bestProduct.productName}" - 딥링크 형식 오류 (등록 스킵): ${deepLink}`);
                  skippedCount++;
                  sendLog('');
                  continue;
                }
                ...
                const product = {
                  productId: `prod_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
                  title: bestProduct.productName,
                  price: bestProduct.productPrice || 0,
                  thumbnail: bestProduct.productImage,
                  deepLink: deepLink,
                  category: bestProduct.categoryName || '상품'
                };

                sendLog(`✅ 상품 발견: ${product.title}`);
                sendLog(`   📁 카테고리: ${product.category}`);
                sendLog(`   🔗 딥링크: ${product.deepLink}`);
                sendLog(`   💰 가격: ${product.price ? product.price.toLocaleString() : 'N/A'}원`);

                // tasks 테이블에 저장
                // ⭐ ID 규칙: UUID 사용
                const taskId = crypto.randomUUID();
                // ⭐ 제목에 [광고] 붙이기 (상품관리 자동화와 동일하게)
                const titleWithAd = `[광고] ${product.title}`;
                // ⭐ 통일 구조 사용
                const productInfo = JSON.stringify(product);
                const settings = JSON.stringify({
                  channel: setting.channel_id,
                  category: '상품',
                  model: 'gemini', // ⭐ 테스트 버튼 기본값: Gemini
                  mediaMode: 'crawl' // ⭐ 테스트 버튼 기본값: 이미지 크롤링
                });

                // v6: task 테이블 최소화 (scheduled_time = NULL, task_queue에 직접 추가)
                const insertTaskSql = getSql('scheduler', 'insertTask');
                await run(insertTaskSql, [taskId, user.userId, null]);

                const insertContentForProductSql = getSql('scheduler', 'insertContentForProduct');
                await run(insertContentForProductSql, [taskId, user.userId, titleWithAd, productInfo, setting.channel_id]);

                const insertContentSettingSql = getSql('scheduler', 'insertContentSetting');
                await run(insertContentSettingSql, [taskId]);

                // ⭐ task_queue는 생성하지 않음 (자동화 스케줄러가 처리)

                // Step 3: 내 목록(coupang_product)에 상품 추가 (이미 위에서 중복 체크 완료)
                sendLog(`📝 Step 3: 내 목록에 상품 추가 중...`);
                try {
                  const coupangProductId = crypto.randomUUID();
                  await run(`
                    INSERT INTO coupang_product (
                      coupang_id, user_id, product_url, deep_link, title, category,
                      thumbnail_url, original_price, discount_price, status, created_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
                  `, [
                    coupangProductId,
                    user.userId,
            ...
            try {
              ...
              if (titles.length > 0) {
                ...
                // ⭐ DB에 저장 (task, content, content_setting, task_queue)
                const taskId = crypto.randomUUID();

                sendLog('');
                sendLog(`💾 DB에 저장 중...`);

                // task 생성 (scheduled_time = NULL, task_queue에 직접 추가)
                const insertTaskSql2 = getSql('scheduler', 'insertTask');
                await run(insertTaskSql2, [taskId, user.userId, null]);

                // content 생성
                const insertContentSql = getSql('scheduler', 'insertContent');
                await run(insertContentSql, [
                  taskId,
                  user.userId,
                  bestTitle.title,
                  'story',
                ...
                // content_setting 생성
                const insertContentSettingSql2 = getSql('scheduler', 'insertContentSetting');
                await run(insertContentSettingSql2, [taskId]);

                // ⭐ task_queue는 생성하지 않음 (자동화 스케줄러가 처리)

                sendLog(`✅ DB 저장 완료! (task_id: ${taskId})`);
