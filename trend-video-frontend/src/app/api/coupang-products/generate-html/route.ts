...
export async function GET(request: NextRequest) {
  try {
    ...
    const products = await db
      .prepare(
        `SELECT id, title, description, category, original_price, discount_price, thumbnail_url, deep_link
    ...
    // 사용자 닉네임 가져오기
    const userInfo = await db.prepare('SELECT nickname FROM user WHERE user_id = ?').get(user.userId) as { nickname?: string } | undefined;
