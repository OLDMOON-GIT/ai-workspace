...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.log('🔍 쿼리: SELECT * FROM content WHERE content_id = ? AND user_id = ?');
    console.log('🔍 파라미터:', { content_id: scriptId, user_id: user.userId });

    const originalScript = await getOne(`
      SELECT *, content_id as contentId FROM content
      WHERE content_id = ? AND user_id = ?
    `, [scriptId, user.userId]) as any;

    console.log('📊 조회 결과:', originalScript ? {
      id: originalScript.id,
      user_id: originalScript.user_id,
      title: originalScript.title,
      promptFormat: originalScript.prompt_format,
    ...
    if (originalScript) {
      ...
      scriptType = originalScript.prompt_format; // prompt_format 컬럼 사용
    ...
    const newScript = await createContent(
      user.userId,
      scriptTitle,  // 제목에 접미사 붙이지 않음
      {
        promptFormat: targetFormat as 'longform' | 'shortform' | 'sora2',
...
- 첫 글자: {, 마지막 글자: }
- scenes 배열에 6개 씬
- 각 씬에 sceneNumber, narration, imagePrompt 포함
...
- scenes 배열에 4개 씬
- 각 씬에 scene_id, soraPrompt, imagePrompt, narration 포함
- metadata에 적절한 정보 설정
- **중요:** 원본 대본의 emotional_arc가 있으면 그대로 복사
