...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRow = await getOne<{ count: number }>(
      `SELECT COUNT(*) as count FROM product_crawl_link_history WHERE user_id = ?`,
      [user.userId]
    );
    ...
    const items = await getAll(
      ...
      FROM product_crawl_link_history
      WHERE user_id = ?
      ORDER BY last_crawled_at DESC, created_at DESC
      LIMIT ? OFFSET ?`,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = id
      ? await run(`DELETE FROM product_crawl_link_history WHERE history_id = ? AND user_id = ?`, [id, user.userId])
      : await run(`DELETE FROM product_crawl_link_history WHERE source_url = ? AND user_id = ?`, [sourceUrl, user.userId]);
