...
export async function POST(request: NextRequest) {
  ...
  try {
    // ⭐ API accepts both 'format' and 'promptFormat' for backward compatibility
    const body = await request.json();
    const { prompt, topic, suggestTitles, productInfo, model, category, userId } = body;
    const promptFormat = body.promptFormat || body.format; // Accept both names

    // 내부 요청인 경우 userId 확인
    if (isInternalRequest) {
      if (!userId) {
        return NextResponse.json(
          { error: '내부 요청에는 userId가 필요합니다.' },
          { status: 400 }
        );
      }
      user = { userId, email: '', isAdmin: false };
      console.log('✅ [INTERNAL] Internal request user:', user.userId);
    }

    /**
     * promptFormat 파라미터: 프롬프트 포맷 (구 videoFormat, format)
...
3. [제목3]`;
    ...
    console.log('🔍 promptFormat 파라미터:', promptFormat);
    console.log('🔍 promptFormat 타입:', typeof promptFormat);

    // 1. 먼저 pending 상태로 대본 생성
    // 상품 타입이거나 product_info.thumbnail이 있으면 카테고리를 '상품'으로 강제 설정
    const isProduct = promptFormat === 'product' || promptFormat === 'product-info' || !!productInfo?.thumbnail;
    const finalCategory = isProduct ? '상품' : category;
    const script = await createScript(
      ...
      promptFormat || 'longform', // 포맷 전달 (기본값: longform)
      finalCategory // 카테고리 전달 (상품은 '상품'으로 강제)
    );

    console.log('📝 대본 생성 작업 시작:', script.id);
    console.log('📝 저장된 promptFormat:', promptFormat || 'longform');
      ...
      try {
        ...
        if (promptFormat === 'product' || promptFormat === 'product-info') {
          if (!productInfo) {
            ...
            console.log('  - deepLink:', productInfo.deepLink);
            ...
            // DB에서 사용자 설정 가져오기
            const db = getDb();
            const userSettings = await db.prepare('SELECT google_sites_home_url, nickname FROM user WHERE user_id = ?').get(user.userId) as { google_sites_home_url?: string; nickname?: string } | undefined;
            ...
            // ⭐ 통일된 구조의 필드를 프롬프트 플레이스홀더에 매핑
            // deepLink → {product_link} 플레이스홀더에 치환
            const beforeReplace = combinedPrompt;
            combinedPrompt = combinedPrompt
              .replace(/{title}/g, productInfo.title || '')
              .replace(/{thumbnail}/g, productInfo.thumbnail || '')
              .replace(/{product_link}/g, productInfo.deepLink || '') // ⭐ deepLink → product_link
...
⚠️ **중요**: 위 상품 정보를 JSON의 product_info 섹션에 **반드시** 포함해주세요.`;
        ...
        const estimatedTotalChars = estimatedLengths[promptFormat || 'longform'] || 33000;

        // SORA2/Product 전용 system prompt (JSON 전용 모드 강제)
        const systemPrompt = (promptFormat === 'sora2' || promptFormat === 'product')
...
START YOUR RESPONSE WITH { NOW.`
        ...
        if (jsonStart !== -1 && jsonEnd !== -1 && jsonEnd > jsonStart) {
          ...
          if (parseResult.success) {
            ...
            if (productInfo && (promptFormat === 'product' || promptFormat === 'product-info')) {
              ...
              if (hasThumbnail || hasProductLink || hasProductDescription || hasHomeUrl || hasNickname) {
                ...
                const userSettings = dbForSettings.prepare('SELECT google_sites_home_url, nickname FROM user WHERE user_id = ?').get(user.userId) as { google_sites_home_url?: string; nickname?: string } | undefined;
              ...
              // ⭐ product_info 섹션을 실제 값으로 직접 교체 (통일된 구조)
              if (parseResult.data.product_info) {
                console.log('🔧 product_info 섹션을 실제 값으로 교체 중...');
                // ⭐ story.json의 product_info도 통일된 구조로 저장
                parseResult.data.product_info = {
                  productId: productInfo.productId || '',
                  title: productInfo.title || '',
                  price: productInfo.price || 0,
                  thumbnail: productInfo.thumbnail || '',
                  deepLink: productInfo.deepLink || '',
                  category: productInfo.category || '상품'
                };
                console.log('✅ product_info 직접 교체 완료:', parseResult.data.product_info);
              }
            }

            // ⭐ metadata에 promptFormat 추가 (image-worker에서 사용)
            if (!parseResult.data.metadata) {
              parseResult.data.metadata = {};
            }
            parseResult.data.metadata.promptFormat = promptFormat;
            console.log(`✅ metadata.promptFormat 설정: ${promptFormat}`);
        ...
        try {
          createApiCost({
            ...
            metadata: {
              ...
              promptFormat: promptFormat || 'longform',
