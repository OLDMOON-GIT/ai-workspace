 ...
 * ║                                                                           ║
 * ║  딥링크 필수 규칙:                                                        ║
 * ║  1. 딥링크가 생성되지 않으면 내 목록(coupang_product)에 절대 저장 불가    ║
 * ║  2. 딥링크 생성 실패 시 큐 상태를 'failed'로 변경하고 저장하지 않음       ║
 ...
 * 쿠팡 파트너스 서명 생성
 *
 * ⚠️ 중요: datetime 형식은 반드시 yymmddTHHMMSSZ (예: 240115T123045Z)
 * ISO 형식(2024-01-15T12:30:45Z)을 사용하면 API 인증 실패!
 */
 ...
 * URL에서 상품 ID 추출
 *
 * ⚠️ 중요: 베스트셀러 affiliate URL은 pageKey 파라미터에 상품 ID가 있음!
 * 예: https://link.coupang.com/re/AFFSDP?lptag=...&pageKey=9118691083&...
 */
function extractProductId(url: string): string | null {
  try {
    const urlObj = new URL(url);

    // 1. pageKey 파라미터에서 추출 (베스트셀러 affiliate 링크)
    const pageKey = urlObj.searchParams.get('pageKey');
    if (pageKey) return pageKey;

    // 2. productId 쿼리 파라미터에서 추출
    const productIdParam = urlObj.searchParams.get('productId');
    if (productIdParam) return productIdParam;

    // 3. URL 경로에서 추출 (/vp/products/{productId})
    const pathMatch = url.match(/\/vp\/products\/(\d+)/);
    if (pathMatch) return pathMatch[1];

    // 4. itemId 파라미터에서 추출 (일부 링크)
 ...
 * 사용자의 쿠팡 파트너스 딥링크 생성
 *
 * ⚠️ 딥링크 생성 실패 시 에러를 던짐 (절대 원본 URL 반환하지 않음!)
 */
async function generateDeepLink(productUrl: string, userId: string): Promise<string> {
  try {
    ...
    // ⚠️ 이미 딥링크인 URL은 처리하지 않음
    if (productUrl.includes('link.coupang.com/')) {
      console.error(`❌ 이미 딥링크인 URL입니다: ${productUrl}`);
      throw new Error('이미 딥링크인 URL은 재변환할 수 없습니다');
    }

    const settings = await loadUserSettings(userId);
    if (!settings || !settings.accessKey || !settings.secretKey) {
      // ⚠️ 중요: 원본 URL 반환하면 안 됨! 에러를 던져서 상품 저장 방지
      throw new Error('쿠팡 API 설정이 없습니다. 설정 페이지에서 API 키를 입력해주세요.');
    }

    // 상품 ID 추출
    const productId = extractProductId(productUrl);
    console.log(`🔍 추출된 상품 ID: ${productId} (원본: ${productUrl})`);
    ...
    // ⚠️ 상품 ID 검증: 숫자만 허용
    if (!/^\d+$/.test(productId)) {
      console.error(`❌ 잘못된 상품 ID 형식 (숫자가 아님): ${productId}`);
      throw new Error(`잘못된 상품 ID 형식: ${productId}`);
    }
    ...
    if (response.ok) {
      ...
      if (data.rCode === '0' && data.data && data.data.length > 0) {
        ...
        console.error(`❌ 요청한 URL: ${cleanUrl}`);
        console.error(`❌ 원본 URL: ${productUrl}`);
        throw new Error(`딥링크 생성 실패 (rCode: ${data.rCode}, rMessage: ${data.rMessage || '알 수 없음'})`);
      }
    } else {
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📦 큐 항목 처리 시작:', queueItem.queue_id);
    ...
    try {
      ...
      await run(`
        ...
        SET
          ...
          updated_at = NOW()
        WHERE queue_id = ?
      `, [JSON.stringify(productInfo), queueItem.queue_id]);
      ...
      if (destination === 'pending_list') {
        ...
        try {
          deepLink = await generateDeepLink(queueItem.product_url, queueItem.user_id);
          ...
          if (!isValidDeepLink) {
            deeplinkFailed = true;
            console.warn(`❌ 딥링크 형식 오류 - 단축 URL만 허용: ${deepLink}`);
          } else {
            console.log(`✅ 딥링크 생성 성공: ${deepLink}`);
          }
        } catch (deeplinkError: any) {
          const errorMsg = deeplinkError.message || '알 수 없는 오류';
          console.error(`❌ 딥링크 생성안됨 (${queueItem.product_url}):`, errorMsg);
          deeplinkFailed = true;
        }

        // ⚠️ 딥링크 생성 실패 시 내 목록에 등록하지 않음
        if (deeplinkFailed || !deepLink) {
          console.log(`🚫 딥링크 생성안됨 - 내 목록 등록 스킵: ${productInfo.title}`);
          // 큐에서 failed 처리
          await run(`
            UPDATE coupang_crawl_queue
            SET status = 'failed', error_message = '딥링크 생성안됨', updated_at = NOW()
            WHERE queue_id = ?
          `, [queueItem.queue_id]);
          return NextResponse.json({
            success: false,
            error: '딥링크 생성안됨 - 쿠팡 API 설정을 확인해주세요.',
        ...
        `, [queueItem.user_id, productInfo.title]) as any;

        if (existingByTitle) {
          console.log(`⏸️ 중복 상품 (제목): ${productInfo.title}`);
          // 큐에서 완료 처리
          await run(`
            UPDATE coupang_crawl_queue SET status = 'completed', updated_at = NOW() WHERE queue_id = ?
          `, [queueItem.queue_id]);
          return NextResponse.json({
            success: false,
            error: `이미 등록된 상품입니다 (제목 중복): ${productInfo.title}`,
            queueId: queueItem.queue_id
          }, { status: 409 });
        }
        ...
        await run(`
          INSERT INTO coupang_product (
            coupang_id, user_id, queue_id, product_url, deep_link, title, description,
        ...
        `, [
          ...
          deepLink, // 사용자의 파트너스 딥링크 또는 원본 URL
          productInfo.title,
          detailedDescription,
          category,
          productInfo.originalPrice || null,
        ...
        console.log(`🔗 링크: ${deepLink}`);
      }

      // 9. 다음 항목이 있는지 확인
      const hasMore = await getOne(`
      ...
      if (newRetryCount >= queueItem.max_retries) {
        ...
        await run(`
          ...
          SET
            ...
            updated_at = NOW()
          WHERE queue_id = ?
        `, [newRetryCount, error?.message, queueItem.queue_id]);
