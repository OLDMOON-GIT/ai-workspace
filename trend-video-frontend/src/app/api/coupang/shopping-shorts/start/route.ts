...
export async function POST(request: NextRequest) {
  try {
    ...
    const initialStatus: TaskStatus = {
      ...
      logs: [`[${getLocalDateTime()}] 쇼핑 쇼츠 자동화 파이프라인 시작`]
    };
    ...
    pythonProcess.stdout?.on('data', async (data) => {
      ...
      currentStatus.logs.push(`[${getLocalDateTime()}] ${log}`);
    ...
    pythonProcess.stderr?.on('data', async (data) => {
      const log = data.toString();
      currentStatus.logs.push(`[${getLocalDateTime()}] ERROR: ${log}`);
      await saveTaskStatus(taskId, currentStatus);
    });
    ...
    pythonProcess.on('close', async (code) => {
      if (code === 0) {
        ...
        try {
          ...
          currentStatus.endTime = getLocalDateTime();
          currentStatus.results = results;
          currentStatus.logs.push(`[${getLocalDateTime()}] 파이프라인 완료 - ${results.length}개 상품 처리됨`);
        } catch (error: any) {
  ...
  try {
    ...
    if (process) {
      ...
      if (status) {
        ...
        status.endTime = getLocalDateTime();
        status.logs.push(`[${getLocalDateTime()}] 작업이 중지되었습니다.`);
