...
export async function GET(request: NextRequest) {
  try {
    ...
    // 통합 조회
    const contents = await getContentsByUserId(user.userId, {
      type: type || undefined,
    ...
    // 전체 개수 (페이징용)
    const allContents = await getContentsByUserId(user.userId, {
      type: type || undefined,
      format: format || undefined
    });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const success = await deleteContent(contentId, user.userId);
