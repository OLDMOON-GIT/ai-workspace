...
/**
 * DELETE /api/social-media/tiktok/accounts?accountId=xxx - TikTok 계정 연결 해제
 */
export async function DELETE(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (success) {
      return NextResponse.json({ success: true, message: 'TikTok 계정 연결이 해제되었습니다' });
    } else {
      return NextResponse.json({ error: '계정 삭제에 실패했습니다' }, { status: 500 });
    }
  } catch (error: any) {
    console.error('TikTok 계정 삭제 실패:', error);
    return NextResponse.json({ error: '계정 삭제에 실패했습니다' }, { status: 500 });
  }
}
