 ...
 *
 * ⚠️ ID 규칙:
 * - task_id = task_schedules.title_id (동일한 값 사용)
...
const queue_manager_1 = require("@/lib/queue-manager");
...
class ImageWorker {
    constructor() {
        ...
        this.manager = new queue_manager_1.QueueManager();
    }
    async start() {
        this.running = true;
            ...
            try {
                ...
                if (this.currentTask) {
                    ...
                    if (shouldRetry) {
                        ...
                        await this.manager.appendLog(taskId, 'image', `⚠️ 재시도 ${this.currentTask.retryCount + 1}/${this.currentTask.maxRetries}: ${error.message}`);
                    }
                    else {
                        // 최대 재시도 초과
                        await this.manager.updateTask(taskId, 'image', {
                        ...
                        await this.manager.appendLog(taskId, 'image', `❌ 실패: ${error.message}`);
                    }
                }
            }
            finally {
    ...
    async processTask(task) {
        ...
        const backendPath = path_1.default.join(process.cwd(), '..', 'trend-video-backend');
        const pythonScript = path_1.default.join(backendPath, 'src', 'image_crawler', 'image_crawler_working.py');
        // 임시 씬 파일 생성
        const tempDir = path_1.default.join(backendPath, 'temp');
        await promises_1.default.mkdir(tempDir, { recursive: true });
        const scenesFilePath = path_1.default.join(tempDir, `scenes_${taskId}.json`);
        await promises_1.default.writeFile(scenesFilePath, JSON.stringify(scenes, null, 2), 'utf-8');
        // 출력 디렉토리 - scriptId 사용 (story.json이 저장된 폴더와 동일)
        // scriptId가 없으면 taskId 사용 (하위 호환성)
        // ⚠️ ID 규칙: prefix 제거하여 순수 ID만 사용
        const folderId = scriptId || taskId;
        const cleanId = folderId.replace(/^(task_|title_|script_)/, '');
        const outputDir = path_1.default.join(backendPath, 'tasks', cleanId);
        ...
        await this.manager.appendLog(taskId, 'image', useImageFX ? '🚀 ImageFX + Whisk 자동화 시작' : '🚀 Whisk 자동화 시작');
        return new Promise((resolve, reject) => {
            ...
            pythonProcess.on('close', async (code) => {
                ...
                // 임시 파일 삭제
                try {
                    await promises_1.default.unlink(scenesFilePath);
                }
                catch (err) {
                ...
                if (code === 0) {
                    ...
                    try {
                        const outputDirExists = await promises_1.default.access(outputDir).then(() => true).catch(() => false);
                        if (!outputDirExists) {
                        ...
                        await this.manager.appendLog(taskId, 'image', `✅ ${imageFiles.length}개 이미지 파일 저장 확인`);
     ...
     */
    async triggerVideoGeneration(taskId) {
        try {
