...
async function updateScenario() {
  ...
  const testCode = `// [자동] admin 페이지 로드 확인
// 자동 감지: app/admin/page.tsx - 페이지가 정상적으로 로드되는지 확인
...
// 페이지 요소 확인 (BTS 바로가기 또는 사용자 관리 카드)
...
`;
  ...
  console.log('Scenario 162 updated successfully');
  await conn.end();
}
