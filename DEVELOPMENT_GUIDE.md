...
개발 과정에서 테스트하거나 임시로 만든 파일들을 관리합니다:

```
temp/
├── 테스트 파일
│   ├── *.py (Python 테스트)
│   ├── *.bat, *.ps1 (Windows 스크립트)
│   └── *.log (로그 파일)
├── 임시 데이터
│   ├── *.txt (메모, 임시 텍스트)
│   ├── *.diff, *.patch (패치 파일)
...
- **git에 커밋되지 않음**: `.gitignore`에 `temp/`가 등록되어 있음
- **프로젝트 코드 아님**: 개발 과정의 임시 산출물
- **필요시만 유지**: 불필요한 파일은 삭제 가능
...
### 기능 확인 프로세스 ⭐⭐⭐
**사용자가 "기능을 어떻게 구현해야 되는지 기능확인을 해봐" 또는 유사한 요청을 할 때:**

1. **기능목록.md 확인** - 먼저 확인할 것!
   - 위치: `trend-video-frontend/기능목록.md`
   - 기능의 동작 흐름
   - 구현 위치 및 파일
   - 현재 완료 상태

2. **스펙 확인** - 자세한 내용 필요 시
   - `PRODUCT_CATEGORY_AUTOMATION_SPEC.md` 등 관련 SPEC 파일 확인
   - API 요청/응답 형식
   - 데이터베이스 스키마

3. **코드 분석** - 실제 구현 확인 필요 시
   - 기능목록.md의 "구현 위치"에 명시된 파일 확인
   - 관련 파일들의 구현 로직 검토

**핵심**: 기능목록.md를 중심으로 모든 개발자(Claude, ChatGPT, Gemini 등)가 같은 목표를 추구할 수 있도록 함
...
17. [Next.js HMR 및 서버 재시작](#17-nextjs-hmr-및-서버-재시작) ⭐⭐⭐
18. [버그 수정 히스토리](#18-버그-수정-히스토리)
...
## 17. Next.js HMR 및 서버 재시작 ⭐⭐⭐

### 🎯 핵심 원칙: 개발 모드에서는 대부분 재시작 불필요

**중요:** Next.js 개발 모드(`npm run dev`)에서는 Hot Module Replacement(HMR)가 작동하므로 **대부분의 코드 변경 시 서버 재시작이 필요 없습니다.**

### ✅ 자동으로 반영되는 것들 (재시작 불필요)
...
- ✅ **브라우저 새로고침만** 하면 즉시 반영
- React Fast Refresh로 상태 유지하며 업데이트
- 재시작 불필요
...
- ✅ **다음 import 시** 자동 반영
- 단, 메모리에 캐시된 경우 새로고침 필요
- 재시작 불필요 (대부분)

### ❌ 재시작이 필요한 경우

#### 1. **이미 실행 중인 백그라운드 프로세스**
...
export function startAutomationScheduler() {
  // 서버 시작 시 한 번 로드되어 계속 실행됨
  schedulerInterval = setInterval(() => {
    processPendingSchedules(); // ← 옛날 코드 계속 사용!
  }, checkInterval);
}
```

**문제:**
- 스케줄러는 서버 시작 시 메모리에 로드
- setInterval로 계속 실행 중
- HMR이 적용되지 않음
...
- ❌ 코드 변경 시 반드시 재빌드 및 재시작 필요
- 빌드된 파일은 정적이므로 HMR 없음

### 📋 실전 가이드

#### 상황 1: API Route 수정
```typescript
// src/app/api/generate-script/route.ts 수정
export async function POST(request: NextRequest) {
...
#### 상황 2: 클라이언트 페이지 수정
```typescript
// src/app/automation/page.tsx 수정
...
#### 상황 3: 스케줄러 로직 수정
```typescript
// src/lib/automation-scheduler.ts 수정
...
브라우저 새로고침(Ctrl+F5)만 하면 반영됩니다.

※ 스케줄러 자동 실행을 테스트하려면 pm2 restart가 필요하지만,
   수동 테스트는 새로고침만으로 가능합니다.
```

### 🔍 재시작 필요 여부 판단 체크리스트

- [ ] `next.config.js` 수정? → 재시작 필요
- [ ] `.env` 파일 수정? → 재시작 필요
- [ ] **스케줄러 로직** 수정? → 재시작 필요 (자동 실행 테스트 시)
- [ ] **setInterval/setTimeout** 사용하는 코드? → 재시작 필요
- [ ] API Route 수정? → 재시작 불필요
- [ ] 클라이언트 컴포넌트 수정? → 재시작 불필요, 새로고침만
- [ ] lib 파일 수정? → 재시작 불필요 (대부분)

### 💡 실전 팁

**스케줄러 없이 기능 테스트하기:**

대부분의 기능은 스케줄러 없이 수동으로 테스트 가능합니다:

```typescript
// 자동화 페이지에서 "즉시 실행" 버튼 클릭
// → API만 호출되므로 재시작 불필요
// → 새 코드로 바로 실행됨
```

**재시작이 정말 필요한 경우에만 명확히 안내:**
...
2. 스케줄러 자동 실행 테스트 (재시작 필요)
   - pm2 restart trend-video-frontend
   - 스케줄 시간 도래 시 자동 실행 확인
```

---

## 18. 사연 영상 생성 시스템 (2025-11-15 추가) ⭐
...
## 20. 상품 자동화 워크플로우 (2025-11-19 추가) ⭐⭐

### 🎯 핵심 원칙: 상품 정보 전체를 포함하여 자동화 등록

**문제:**
- 자동화 큐에 상품을 등록할 때 `title`만 넘기면, 이후 대본 생성 단계에서 해당 상품의 상세 정보(이미지, 가격, URL 등)를 활용할 수 없음.
- 결과적으로 "이 상품은..."과 같이 맥락 없는 대본이 생성됨.

**해결:**
- 자동화 큐(`video_titles` 테이블)에 등록할 때, 상품의 모든 관련 데이터를 `product_data` 컬럼에 JSON 형태로 저장해야 함.

### ✅ 올바른 워크플로우

1.  **상품 소싱**: '쿠팡 베스트' 등의 소스에서 상품을 선택합니다.
2.  **딥링크 생성**: 해당 상품의 트래킹 가능한 딥링크를 생성합니다.
3.  **'내 목록' 등록**: 생성된 딥링크와 함께 상품을 '내 목록' 또는 '상품 관리' 페이지에 추가합니다. 이 때, 상품명, 이미지 URL, 가격 등 최대한 많은 정보를 함께 저장합니다.
4.  **자동화 등록**:
    - '내 목록'에서 '자동화' 버튼을 클릭합니다.
    - 이 때, `video_titles` 테이블에 새로운 row를 추가하면서, **3번 단계에서 저장했던 모든 상품 정보를 JSON으로 변환하여 `product_data` 컬럼에 저장**합니다.
    - `title` 컬럼에는 상품명을 저장합니다.

**데이터 흐름:**

```
[상품 관리 페이지]                                      [video_titles 테이블]
- productName: "워더미 겨울 양말"                       - title: "워더미 겨울 양말"
- productImage: "image.jpg"         --+              - type: "product"
- productUrl: "link.coupang.com/..."  |                - product_data: '{
- price: 12900                        | (자동화 등록)        "productName": "워더미 겨울 양말",
...
- '자동화 등록' 시 `title`만 `video_titles` 테이블에 저장하고 `product_data`를 누락시키는 경우.
- 이렇게 하면 대본 생성 AI가 상품 정보를 전혀 알 수 없어 고품질의 상품 설명 대본을 생성할 수 없습니다.

### 🔍 관련 코드

- **프론트엔드**: `trend-video-frontend/src/app/admin/coupang-products/page.tsx`의 `handleAddToAutomation` 함수. `localStorage` 또는 상태(state)에서 `current_product_data`를 가져와 API로 전송해야 함.
- **백엔드 API**: `trend-video-frontend/src/app/api/titles/route.ts`의 `POST` 핸들러. 요청 본문에서 `productData`를 받아 `addVideoTitle` 함수로 전달해야 함.
- **데이터베이스 로직**: `trend-video-frontend/src/lib/automation.ts`의 `addVideoTitle` 함수. `productData` 파라미터를 받아 `video_titles` 테이블에 저장함.

```typescript
// src/lib/automation.ts - addVideoTitle 함수
export function addVideoTitle(data: {
  // ... 다른 파라미터들
  productData?: string; // ✅ 이 파라미터가 핵심
  // ...
}) {
  // ...
  db.prepare(`
