...
2. **MySQL에서 조회**: `SELECT * FROM bugs WHERE id = {숫자}`
3. **상태 확인**:
   - `status = 'in_progress'`이면 `assigned_to` 확인
   - `assigned_to`가 본인이면 → 작업 계속
   - `assigned_to`가 다른 사람이면 → **작업 금지, 다른 버그로 이동**

### ⛔⛔⛔ bugs 테이블은 MySQL에만 존재! SQLite 아님! ⛔⛔⛔

**bugs 테이블 조회 시 반드시 MySQL 사용!**
...
async function run() {
  ...
  const [rows] = await conn.execute('SELECT * FROM bugs WHERE id = ?', [3025]);
...
# ❌ 잘못된 방법 (SQLite - bugs 테이블 없음!)
node -e "const Database = require('better-sqlite3'); ..."  // 에러 발생!
```

### JavaScript 코드 예시
```javascript
// BTS-3018 → 3018 (type 컬럼으로 bug/spec 구분)
const bugId = input.replace(/^BTS-/i, '');

// MySQL에서 조회 (SQLite 아님!)
const mysql = require('mysql2/promise');
const conn = await mysql.createConnection({
  host: 'localhost',
  user: 'root',
...
// 진행 중 확인
if (bug.status === 'in_progress' && bug.assigned_to !== myProcessId) {
  console.log(`${bug.assigned_to}가 작업 중 - 건너뛰기`);
  return;
}
```
...
```sql
-- 버그 등록 예시 (id는 AUTO_INCREMENT!)
INSERT INTO bugs (title, summary, status, priority, type, metadata, created_at, updated_at)
VALUES ('버그 제목', '버그 요약', 'open', 'P0', 'bug', '{}', NOW(), NOW());

-- 해결 후
UPDATE bugs SET status = 'resolved', updated_at = NOW() WHERE id = 3027;
```
...
```
## ���� �޸� (Log Monitor/Bug Manager)
- SQLite `~/.mcp-debugger/error-queue.db`: �α� ����Ͱ� �ǽð� �����ϴ� �ӽ� ���� ť(Inbox). ���� pending/processing/claimed/resolved/ignored. �ߺ� ���͸����ڵ� ����(30�� ���� processing��pending)��.
- MySQL `trend_video.bugs`: ���� BTS ����. ���� open/in_progress/resolved/closed. �켱����/�����/�ذ��Ʈ ����.
- �帧: �α� �� SQLite error_queue �� (triage ��) �ʿ��� �� MySQL bugs ���. �� ������ ������ �ٸ��� �ڵ� ��ũ���� ����.
