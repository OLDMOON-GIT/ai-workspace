...
// Old pattern - the entire disabled block (using detected line ending)
const oldBlock = [
...
console.log('Pattern found:', content.includes(oldBlock));
...
if (content.includes(oldBlock)) {
  ...
  const lines = oldBlock.split(lineEnding);
  for (let i = 0; i < lines.length; i++) {
    if (!content.includes(lines[i])) {
      console.log(`Line ${i} NOT FOUND: "${lines[i].substring(0, 60)}..."`);
      break;
    }
  }
}
