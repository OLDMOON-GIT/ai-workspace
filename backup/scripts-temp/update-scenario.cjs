...
async function main() {
  ...
  const newTestCode = `// [자동] charge-requests 페이지 로드 확인 - 기본 시나리오
// 관리자 로그인 후 충전 요청 페이지가 정상 로드되는지 확인
...
async function login() {
  ...
  if ((await remember.count()) > 0) {
    try { await remember.first().check(); } catch (e) {}
  }
...
// 충전 요청 페이지로 이동
await page.goto(BASE_URL + '/admin/charge-requests', { waitUntil: 'domcontentloaded' });
...
`;
  ...
  await conn.execute(
    'UPDATE test_scenario SET test_code = ? WHERE scenario_id = 179',
    [newTestCode]
  );
  console.log('시나리오 179 업데이트 완료');
  await conn.end();
}
