...
async function updateScenario() {
  ...
  const testCode = `// [자동] /admin/mcp-debugger - 페이지 로드 확인
// 관리자 로그인 후 MCP Debugger 페이지 로드 확인
...
// 페이지 헤더 확인
await page.waitForSelector('text=MCP Debugger Console', { timeout: 15000 });
`;
  ...
  console.log('Scenario 200 updated successfully');
  await conn.end();
}
